...
|---|---|---|
| **sensing** | `tick_secs` [10,86400], `wait_low_ms` [1,60000], `wait_high_ms` [1,600000], `probe_cap` [1,10000], `expansion_enabled` {1=on,2=off}, `target_util_pct` [50,95], `min_scan_rate_milli` [10,2000], `value_clamp_r` [1,16]\*, `inflight_cap` [1,8]\*, `capital_multiplier_k_milli` [0,10000], `capex_reserve_credits` [0,5M], `quartermaster_cadence_secs` [300,86400], `surge_inflight_cap` [1,64], `pressure_half_life_secs` [1,3600]\* | the parked-probe sensing engine: cadence, fleet ceiling, scan pacing, the buy floor's two terms. \* = binds on rebuild |
...
**Master switches are encoded 1=on / 2=off, not 0/1** (`growth_enabled`, `expansion_enabled`),
precisely because `tune <key> 0` is the fleet-wide revert-to-default verb and would otherwise make
"off" unexpressible. Read a switch before assuming its polarity; new *behaviour* flags may still
