Feature: Route Entity
  ...
  Scenario: Route segment repr shows segment info
    ...
    Then the representation should contain "X1-A1"
    And the representation should contain "X1-B2"
    And the representation should contain "100"

  Scenario: Route segment repr includes refuel marker when needed
    Given a route segment from "X1-A1" to "X1-B2" requiring refuel
    When I get the segment representation
    Then the representation should contain "REFUEL"

  # Route Initialization Tests
  Scenario: Create route with valid data
    Given a route segment from "X1-A1" to "X1-B2"
    And a route segment from "X1-B2" to "X1-C3"
  ...
  Scenario: Cannot create route with disconnected segments
    Given a route segment from "X1-A1" to "X1-B2"
    And a route segment from "X1-A1" to "X1-C3"
    When I attempt to create a route with disconnected segments
    Then the route creation should fail with "Segments not connected"
  ...
  Scenario: Cannot create route when segment exceeds fuel capacity
    ...
    Then the route creation should fail with "requires 150 fuel but ship capacity is 100"
  ...
  Scenario: Cannot start execution when already executing
    ...
    Then the operation should fail with "Cannot start route in status EXECUTING"

  Scenario: Cannot start execution when completed
    Given a route in "COMPLETED" status
    When I attempt to start route execution
    Then the operation should fail with "Cannot start route in status COMPLETED"
  ...
  Scenario: Cannot complete segment when not executing
    ...
    Then the operation should fail with "Cannot complete segment when route status is PLANNED"
  ...
  Scenario: Repr contains route info
    ...
    Then the representation should contain "route-1"
    And the representation should contain "SHIP-1"
    And the representation should contain "2"
    And the representation should contain "PLANNED"

  Scenario: Repr updates with status
    Given a route in "PLANNED" status
    When I start route execution
    And I get the route representation
    Then the representation should contain "EXECUTING"
