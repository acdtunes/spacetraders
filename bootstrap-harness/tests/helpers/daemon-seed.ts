...
//   3. Contract-hauler dedication (Option B). The twin now seeds the contract income haulers as REAL
//      `world.ships` (twin/src/world/store.ts seedGateEntry), so `GET /my/ships` returns them and the
//      daemon syncs them on boot — the prod invariant that a :5434 ship is a SUBSET of /my/ships. Under
//      Option B the coordinator BUYS the gate-delivery fleet from these haulers' income (it NEVER
//      repurposes them) and flies one idle contract hauler to the yard to EXECUTE each buy, so the
//      purchaser MUST be a ship the twin knows or the /v2 navigate + set-flight-mode + purchase-ship
//      404. But the SpaceTraders API has no concept of the bot's dedicated_fleet tag, so a synced hauler
//      lands untagged → `obs.Haulers` (:5434 ships tagged dedicated_fleet='contract') would be EMPTY. We
//      therefore PRE-SEED the same N symbols with dedicated_fleet='contract'; SyncAllFromAPI refreshes
//      their nav/frame/fuel from the twin but PRESERVES dedicated_fleet (sp-bi75), so after boot each
//      reads as an idle contract hauler. The two seed sides AGREE on symbols/count/location: the same
//      `<agent>-H<i>` symbols, the same count (fixture.haulers), the same home marketplace (= the fixture
//      HQ, where the twin parks them).
//
// CALL SITE: between `resetDaemonDb()` and `startTestDaemon()` in scenario-gate.ts (mirrors the INCOME
// wiring). The twin's world.ships is the source of truth; this seed only adds the daemon-local
// dedicated_fleet tag the API cannot carry.
...
export async function seedDaemonGateEntry(fixture: GateFixture = {}): Promise<SeedGateEntryResult> {
  ...
  // (3) Contract-hauler dedication — PRE-TAG the same N `<agent>-H<i>` symbols the twin now serves as real
  // world.ships (store.ts seedGateEntry) with dedicated_fleet='contract'. The API can't carry that tag, so
  // without this pre-seed the boot sync lands them untagged and obs.Haulers is empty; SyncAllFromAPI
  // PRESERVES dedicated_fleet (sp-bi75) while refreshing nav/frame/fuel from the twin, so after boot each
  // reads as an idle contract hauler. Under Option B these STAY contract earners the whole GATE run: the
  // coordinator BUYS the gate-delivery fleet from their income (never repurposes them). Each carries NO
  // container assignment, so IsIdle()==true and it DOUBLES as an idle purchaser (obs.HasIdlePurchaser,
  // bootstrap_ports.go): the coordinator flies one to the yard to execute each buy and it returns still
  // contract-tagged (only the newly bought hulls are manufacturing-tagged gate workers). The row's
  // nav/frame/fuel MATCH the twin hull (HAULER, FRAME_LIGHT_FREIGHTER, 400/400 fuel, 40 cargo) and it is
  // parked at the same home marketplace (= the fixture HQ) so the two seed sides agree pre-boot too.
  const homeMarket =
    scalar(
