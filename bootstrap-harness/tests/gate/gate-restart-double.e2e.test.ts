...
describe('bootstrap GATE — double (back-to-back) restart idempotency', () => {
  it('two rapid reboots mid-construction change nothing: sticky GATE, guards once, no re-buy, completes', async () => {
    ...
    await resetDaemonDb();
    // Daemon-local GATE seed (coverage + income + gate-site + contract haulers), as the non-restart gate
    // specs do. resetDaemonDb() runs ONCE (preserves players/eras); the seed PERSISTS across all THREE
    // lifetimes (initial + two reboots), so each rebooted brain re-derives sticky GATE instead of DATA.
