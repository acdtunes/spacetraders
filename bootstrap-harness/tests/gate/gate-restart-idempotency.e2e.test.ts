...
import { seedDaemonGateEntry } from '../helpers/daemon-seed';
...
describe('bootstrap GATE — restart idempotency', () => {
  it('no double-start / no re-bounce / no double-worker-buy / no double-autosizer across a mid-GATE restart', async () => {
    const fixture = gateEntry({ haulers: 2, gateMaterialChains: 4, credits: 3_000_000 });
    await twinGate.seedGate(fixture);
    await resetDaemonDb();
    // Seed the daemon-local GATE world (DATA-complete coverage + realized income over the bar + the
    // JUMP_GATE site row + the contract-hauler pool), exactly as the non-restart gate specs do via
    // withGateScenario. resetDaemonDb() runs ONCE (it preserves players/eras), and the daemon reboot does
    // NOT wipe :5434, so this seed PERSISTS across BOTH lifetimes — without it the rebooted brain observes
    // coverage=0, derives DATA, and buys scout probes instead of re-deriving sticky GATE.
    await seedDaemonGateEntry(fixture);
