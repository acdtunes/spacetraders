...
describe('bootstrap INCOME — restart mid-TRANSIT (in-flight hull re-adoption)', () => {
  it('re-adopts a hauler that was in flight at the kill: arrival acted on, parked, never re-bought', async () => {
    ...
    try {
      launchBootstrap();
      // Lifetime 1: run until the FIRST contract hauler has been bought AND dispatched to a hub — i.e.
      // it is genuinely IN_TRANSIT carrying a real route.arrival. The old count-only predicate
      // (PurchaseShip>=1 && navigate>=1) fired at navigate=1 — the PURCHASER's own navigate to the
      // shipyard — so the just-bought hull was still DOCKED there, NOT in flight (arrivalBefore null in
      // ~60% of runs). Pinning nav.status==IN_TRANSIT reproduces the spec's real precondition ("still EN
      // ROUTE") every run, so the crash below always lands mid-flight and the adopt-not-redispatch tooth
      // bites (arrivalBefore present). The coordinator blocks on this hull's arrival before buying the
      // next, so PurchaseShip is still exactly 1 throughout its flight (asserted below).
      const afterFirst = await pollUntil(
        () => twinIncome.incomeState(),
        (s) => {
          if (countCall(s.mutationLog, 'PurchaseShip') < 1) return false;
          const h = s.haulers[0];
          const ship = h && s.ships.find((x) => x.symbol === h.symbol);
          return !!ship && ship.nav.status === 'IN_TRANSIT' && !!ship.nav.route?.arrival;
        },
        { steps: 120, stepMs: 250, advanceMs: 1000 },
      );
      ...
      expect(arrivalBefore, 'the in-flight hull carries a real transit at the kill').toBeTruthy();

      // CRASH the daemon (SIGKILL) while the hull is still en route — its in-memory arrival timer dies
      // WITHOUT completing, so the NEXT daemon must re-adopt the in-flight hull (ScheduleAllPending
      // re-arm) to act on its arrival. A graceful stop() (SIGTERM) would instead let this daemon drain
      // ~30s, finish the arrival itself AND keep buying/placing more haulers — defeating the mid-flight
      // kill and (via that messy drain) colliding two haulers onto one hub. See finally: still stop().
      await daemon.kill();
