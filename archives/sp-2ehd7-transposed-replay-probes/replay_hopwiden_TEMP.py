...
def systems_within_hops(home, neighbors, hop_depth):
    """BFS over the gate-adjacency `neighbors` map from `home` out to `hop_depth`
    gate hops. Returns the set of systems reachable in <= hop_depth hops, ALWAYS
    including `home` (home is depth 0). hop_depth <= 0 yields just {home}.
    `reached` only grows layer by layer, so systems_within_hops(N+1) ⊇ (N)."""
    reached = {home}
    frontier = {home}
    for _ in range(max(0, hop_depth)):
        nxt = set()
        for s in frontier:
            nxt |= neighbors.get(s, set())
        nxt -= reached
        if not nxt:
            break
        reached |= nxt
...
def compute_allowed(home, neighbors, by_system, hop_depth):
    """Candidate systems = (systems within `hop_depth` gate hops of home) ∩ (systems
    with a market present in this snapshot) ∪ {home}.

    DEPTH-1 EQUIVALENCE (verified): systems_within_hops(home, ., 1) = {home} ∪ neighbors[home],
    so compute_allowed(., 1) = {home} | (({home} ∪ neighbors[home]) & by_system)
    = {home} | (neighbors[home] & by_system) — byte-identical to the historical
    `allowed = {home} | (neighbors.get(home, set()) & set(by_system))` at every site.
    Depth N ⊇ depth (N-1) because systems_within_hops grows monotonically."""
...
def _solve_rate_case(snapshot, waypoints, home, allowed, hold, max_spend, reserve,
                     version, cap):
    """ONE rate-objective solve at the given distinct-system cap (lean half of run_case,
    which always solves profit AND rate). Used by widen_pass so a multi-cap sweep costs
    one solve per cap-cell, not two — halving solve count vs the --arm matrix while
...
def widen_pass(samples, rows, neighbors, coords, hulls, version, caps,
               max_spend, reserve, hop_depth):
    """Lean RATE-only multi-cap sweep at a fixed candidate-hop-depth. Per (sample, home,
    hull) it solves the SAME reconstructed snapshot once per cap in `caps`, all at the
    RATE objective (the armed longer-tour objective). Snapshot/allowed/ship assembly
    mirrors arming_pass exactly, so cph numbers are apples-to-apples with the --arm gate.
    A case is emitted only when EVERY cap cell solved. Returns dicts of
    dict(sample, home, hold, results_by_cap={cap: result})."""
    cases = []
    for sample_t in samples:
        ...
        for home, markets in sorted(by_system.items()):
            if len(markets) < 2:
                continue
            allowed = compute_allowed(home, neighbors, by_system, hop_depth)
            for hold in hulls:
                cells = {}
                for cap in caps:
...
def widen_verdict(cases, caps, overhead_seconds):
    """Fleet-$/hr per cap over the JOINT-feasible case set (every cap recovers a
    positive plan_seconds), plus each cap's delta vs the FIRST cap in `caps` (the
    baseline). All cph delegate to fleet_cph — the one definition summarize prints."""
    joint = [c for c in cases
...
def arming_pass(samples, rows, neighbors, coords, hulls, version,
                baseline_cap, candidate_cap, max_spend, reserve, hop_depth=1):
...
def closure_ab_pass(samples, rows, neighbors, coords, hulls, version,
                    cap, k, max_spend, reserve, hop_depth=1):
...
def main():
    ...
    # sp-7q5t candidate-widening probe (offline evidence only). Default 1 keeps EVERY
    # allowed-set byte-identical to today's live 1-gate-hop shape; N>1 does a BFS over the
    # gate-adjacency map out to N hops before intersecting with markets-present. Applied at
    # all three allowed-construction sites (one_pass, arming_pass, closure_ab_pass).
    ap.add_argument("--candidate-hop-depth", type=int, default=1,
                    help="gate-hop BFS radius for candidate systems (1 = live default, "
                         "byte-identical to home + 1-hop neighbors)")
    ap.add_argument("--verify-bfs", action="store_true",
                    help="print allowed-set sizes at depth 1/2/3 for sample homes + confirm "
                         "depth-1 == the historical {home}|(neighbors&by_system) set, then exit")
    # sp-7q5t lean rate-only multi-cap widen sweep (offline). Half the solves of --arm
    # (one rate solve per cap-cell), computes cap-N RATE fleet-$/hr + deltas vs the first
    # cap. Combined with --candidate-hop-depth it yields BOTH the per-depth cap-4 rate cph
    # (the widening table) and the cap-4-vs-cap-2 rate delta (the cap-under-widening gate,
    # == --arm's gating true_live_delta_pct) at half the runtime cost.
    ap.add_argument("--widen", action="store_true",
                    help="run the lean rate-only multi-cap widen sweep at --candidate-hop-depth")
    ap.add_argument("--widen-caps", default="2,4",
                    help="comma caps for the widen sweep (first = baseline for the deltas)")
    ...
    if args.verify_bfs:
        # Fast, solve-free BFS verification (deliverable evidence). Uses the NEWEST sample's
        # reconstructed snapshot for by_system — representative of the live snapshot shape.
        snap = reconstruct_snapshot(rows, samples[-1])
        by_system = defaultdict(set)
        for s in snap:
            by_system[s["system_symbol"]].add(s["waypoint_symbol"])
        homes = sorted(h for h, m in by_system.items() if len(m) >= 2)
        # (1) EXHAUSTIVE depth-1 == historical-formula check over every eligible home.
        mismatches = []
        for home in homes:
            orig = {home} | (neighbors.get(home, set()) & set(by_system))
            d1 = compute_allowed(home, neighbors, by_system, 1)
            if orig != d1:
                mismatches.append(home)
        print(f"=== BFS VERIFICATION (snapshot @ {samples[-1]}, {len(homes)} eligible homes) ===")
        print(f"depth-1 == historical {{home}}|(neighbors&by_system): "
              f"{'PASS (all match)' if not mismatches else 'FAIL: ' + ','.join(mismatches[:10])}")
        # (2) Monotonic-growth counts for a handful of sample homes (most-connected first, so
        #     the growth is visible). Assert depth3 >= depth2 >= depth1 per home.
        ranked = sorted(homes, key=lambda h: len(neighbors.get(h, set())), reverse=True)
        grow_ok = True
        print(f"\n{'home':<12} {'d1':>5} {'d2':>5} {'d3':>5}   (|allowed| = systems within N hops ∩ markets ∪ home)")
        for home in ranked[:8]:
            sizes = [len(compute_allowed(home, neighbors, by_system, d)) for d in (1, 2, 3)]
            if not (sizes[0] <= sizes[1] <= sizes[2]):
                grow_ok = False
            flag = "" if sizes[0] <= sizes[1] <= sizes[2] else "  <-- NON-MONOTONIC!"
            print(f"{home:<12} {sizes[0]:>5} {sizes[1]:>5} {sizes[2]:>5}{flag}")
        # (3) Fleet-wide growth totals (sum |allowed| across all eligible homes per depth).
        tot = {d: sum(len(compute_allowed(h, neighbors, by_system, d)) for h in homes)
               for d in (1, 2, 3)}
        print(f"\nfleet total |allowed| across {len(homes)} homes:  "
              f"depth1={tot[1]}  depth2={tot[2]}  depth3={tot[3]}")
        print(f"strictly grows d1<=d2<=d3 (all homes): {'PASS' if grow_ok else 'FAIL'}")
        print(f"\nVERDICT: {'GREEN' if (not mismatches and grow_ok) else 'RED — BFS INVALID, DO NOT TRUST $/hr'}")
        return 0 if (not mismatches and grow_ok) else 1

    if args.widen:
        # Lean rate-only multi-cap widen sweep at the requested hop depth. Early-exit BEFORE
        # one_pass so the default profit-vs-rate pass's solves don't blow the time budget.
        widen_caps = [int(c) for c in args.widen_caps.split(",") if c]
        t0 = time.time()
        wcases = widen_pass(samples, rows, neighbors, coords, hulls, version,
                            widen_caps, args.max_spend, args.reserve,
                            args.candidate_hop_depth)
        wv = widen_verdict(wcases, widen_caps, args.tour_overhead_seconds)
        print(f"\n=== WIDEN SWEEP (candidate-hop-depth {args.candidate_hop_depth}, "
              f"RATE, caps {widen_caps}, hulls {hulls}, samples {args.samples}) ===")
        print(f"joint-feasible cases (all caps solved): {wv['cases']}")
        for cap in widen_caps:
            print(f"  cap {cap:>2} rate: {wv['cph_by_cap'][cap]:>12,.0f} cr/hr"
                  f"   (delta vs cap {wv['baseline_cap']}: {wv['delta_by_cap'][cap]:+7.2f}%)")
        print(f"[widen_pass wall: {time.time() - t0:.1f}s over {len(wcases)} solved cases]")
        return 0
    ...
    def one_pass(label):
        ...
        for sample_t in samples:
            ...
            for home, markets in sorted(by_system.items()):
                if len(markets) < 2:
                    continue
                allowed = compute_allowed(home, neighbors, by_system,
                                          args.candidate_hop_depth)
                for hold in hulls:
                    res = run_case(snapshot, waypoints, home, allowed, hold,
    ...
    if args.arm:
        ...
        cases = arming_pass(samples, rows, neighbors, coords, hulls, version,
                            ...
                            hop_depth=args.candidate_hop_depth)
