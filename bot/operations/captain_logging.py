...
class CaptainLogWriter:
    ...
    def __init__(self, agent_callsign, token=None):
        ...
        # Paths - store logs in agents/{agent}/ directory
        self.agent_dir = Path(__file__).parent.parent / "agents" / agent_callsign.lower()
        self.log_file = self.agent_dir / "docs" / "captain-log.md"
        self.sessions_dir = self.agent_dir / "logs" / "sessions"
        self.reports_dir = self.agent_dir / "logs" / "executive_reports"

        # Create directories
        self.agent_dir.mkdir(parents=True, exist_ok=True)
        (self.agent_dir / "docs").mkdir(exist_ok=True)
...
def captain_log_operation(args):
    ...
    from operations.common import setup_logging
