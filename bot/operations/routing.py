...
from routing import GraphBuilder, RouteOptimizer, TourOptimizer, TimeCalculator
...
def scout_markets_operation(args):
    ...
    while running:
        ...
        # Load or build graph
        graph_file = Path('graphs') / f"{args.system}_graph.json"

        if not graph_file.exists():
            logger.info(f"Graph file not found, building graph for {args.system}...")
            builder = GraphBuilder(api)
            graph = builder.build_system_graph(args.system)
            if not graph:
