"""SQLAlchemy-based WaypointRepository implementation for caching waypoint data."""
...
class WaypointRepositorySQLAlchemy(IWaypointRepository):
    """SQLAlchemy implementation of waypoint caching repository"""

    TTL_SECONDS = 7200  # 2 hours

    def __init__(self, engine: Engine, api_client_factory=None):
        """
    ...
    def save_waypoints(self, waypoints_list: List[Waypoint], synced_at: Optional[datetime] = None, replace_system: bool = False) -> None:
        """
        Save or update waypoints in cache using UPSERT with timestamp.

        Args:
            waypoints_list: List of Waypoint value objects to cache
            synced_at: Timestamp when waypoints were synced (defaults to now)
            replace_system: If True, delete all existing waypoints for the system first
        """
        ...
        system_symbol = waypoints_list[0].system_symbol

        with self._engine.begin() as conn:
            # Clear old waypoints for the system if replace_system is True
            if replace_system:
                conn.execute(
                    delete(waypoints).where(waypoints.c.system_symbol == system_symbol)
                )
            ...
            for waypoint in waypoints_list:
                traits_json = json.dumps(waypoint.traits) if waypoint.traits else None
                orbitals_json = json.dumps(waypoint.orbitals) if waypoint.orbitals else None

                if backend == 'postgresql':
                    stmt = pg_insert(waypoints).values(
                        ...
                        y=waypoint.y,
                        traits=traits_json,
                        has_fuel=1 if waypoint.has_fuel else 0,
                        orbitals=orbitals_json,
                        synced_at=sync_time_str
                    )
                    stmt = stmt.on_conflict_do_update(
                        index_elements=['waypoint_symbol'],
                        set_={
                            'system_symbol': stmt.excluded.system_symbol,
                            'type': stmt.excluded.type,
                            'x': stmt.excluded.x,
                            'y': stmt.excluded.y,
                            'traits': stmt.excluded.traits,
                            'has_fuel': stmt.excluded.has_fuel,
                            'orbitals': stmt.excluded.orbitals,
                            'synced_at': stmt.excluded.synced_at
                        }
                    )
                else:
                    stmt = sqlite_insert(waypoints).values(
                        waypoint_symbol=waypoint.symbol,
                        system_symbol=waypoint.system_symbol,
                        type=waypoint.waypoint_type,
                        x=waypoint.x,
        ...
        logger.debug(f"Saved {len(waypoints_list)} waypoints to cache with timestamp {sync_time_str}")
    ...
    def _query_from_database(self, system_symbol: str) -> List[Waypoint]:
        ...
        with self._engine.connect() as conn:
            stmt = (
                select(
                    waypoints.c.waypoint_symbol,
                    waypoints.c.system_symbol,
                    waypoints.c.type,
                    waypoints.c.x,
                    waypoints.c.y,
                    waypoints.c.traits,
                    waypoints.c.has_fuel,
                    waypoints.c.orbitals
                )
                .where(waypoints.c.system_symbol == system_symbol)
                .order_by(waypoints.c.waypoint_symbol)
            )
    ...
    def find_by_trait(self, system_symbol: str, trait: str, player_id: Optional[int] = None) -> List[Waypoint]:
        ...
        with self._engine.connect() as conn:
            stmt = (
                ...
                .where(
                    waypoints.c.system_symbol == system_symbol,
                    waypoints.c.traits.like(f'%"{trait}"%')
                )
                .order_by(waypoints.c.waypoint_symbol)
            )
            ...
            result = conn.execute(stmt)
            rows = result.fetchall()
            waypoints_list = [self._row_to_waypoint(row) for row in rows]

            # Filter in Python to ensure exact trait match (JSON contains the trait)
            return [wp for wp in waypoints_list if trait in wp.traits]
    ...
    def get_system_sync_time(self, system_symbol: str) -> Optional[datetime]:
        ...
        with self._engine.connect() as conn:
            stmt = (
                select(func.max(waypoints.c.synced_at).label('last_sync'))
                .where(waypoints.c.system_symbol == system_symbol)
            )
