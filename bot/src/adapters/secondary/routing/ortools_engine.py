...
class ORToolsRoutingEngine(IRoutingEngine):
    ...
    def optimize_fleet_tour(
        ...
        # Any market the VRP could not place — a genuinely unreachable outlier, or a
        # drop under a tight solve budget — is distributed across the ships rather
        # than dropped. Single-ship scouting keeps every market, so a 2+ ship
        # partition must too: never silently shrink the book, raise, or collapse to an
        # empty partition of 0 tours (sp-t73c). The VRP has already optimised the
        # placement of everything it could route; this only tops up the remainder,
        # preserving input order for determinism.
        unplaced_markets = [m for m in markets if m not in assigned_waypoints]
        if unplaced_markets:
            logger.warning(
                "VRP left %d of %d market(s) unplaced (likely unreachable outliers); "
    ...
    def _distribute_evenly(
        ...
        balanced when the VRP cannot place every market (an unreachable outlier)
        or returns no solution at all. Ships are compared by current market count
        with a stable tie-break on fleet order, so the result is reproducible.
        """
        if not assignments:
            return
