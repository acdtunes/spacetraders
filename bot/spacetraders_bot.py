...
Usage:
    # Mining operation
    python3 spacetraders_bot.py mine --token TOKEN --ship SHIP-1 --asteroid X1-HU87-B9 --market X1-HU87-B7 --cycles 30

    # Market scouting
    python3 spacetraders_bot.py scout-markets --player-id PLAYER_ID --ship SHIP-2 --system X1-HU87 --markets 20

    # Trading operation
    python3 spacetraders_bot.py trade --token TOKEN --ship SHIP-3 --route-file routes.json --cycles 10

    # Contract fulfillment
    python3 spacetraders_bot.py contract --token TOKEN --ship SHIP-1 --contract-id ID
...
# Import all operations from operations package
...
def main():
    ...
    contract_parser.add_argument('--buy-from', help='Waypoint to buy from (optional)')
    contract_parser.add_argument('--mine-from', help='Asteroid to mine from (optional)')
    contract_parser.add_argument('--procurement-mode', choices=['mine', 'procure'], help='Override procurement strategy (mine/procure)')
    ...
    if args.operation == 'mine':
        ...
        from operations import navigate_ship
        return navigate_ship(args, api, logger)
    elif args.operation == 'plan-market-route':
        from operations.routing import scout_markets_operation as plan_route_operation
        return plan_route_operation(args)
    elif args.operation == 'scout-markets':
        from operations.routing import scout_markets_operation
