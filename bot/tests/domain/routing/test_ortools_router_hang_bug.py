...
def test_ortools_router_does_not_hang_on_large_graph():
    """Test that router completes routing in reasonable time (no hang) for medium graph."""
    # Build smaller test graph (20 waypoints instead of 88 for faster testing)
    graph = build_large_dense_graph(num_waypoints=20)
...
def test_ortools_router_edge_lookup_performance():
    ...
    # Should complete 100 lookups quickly (if properly cached)
    assert elapsed < 0.1, f"100 edge lookups took {elapsed:.3f}s - caching not working!"
    print(f"✓ 100 edge lookups in {elapsed:.3f}s (caching works)")


def test_dijkstra_pathfinding_performance():
    """Test that Dijkstra pathfinding (_find_waypoint_path) is efficient."""
    # Build small test graph (15 waypoints instead of 40 for faster testing)
    graph = build_large_dense_graph(num_waypoints=15)
    symbols = sorted(graph["waypoints"].keys())
    ...
    # Should complete quickly even with 15 waypoints
    assert elapsed < 1.0, f"Dijkstra took {elapsed:.2f}s (expected <1s) - O(E³) complexity detected!"
