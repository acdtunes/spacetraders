...
// The other reason a directory holding .go files is absent from the census: every
// file in it is GENERATED, which AnalyzeFile drops. The calibration has to agree, or
// regenerating a proto fails the gate with a message about directory naming.
func TestGeneratedOnlyDirIsNotReadAsAMissingPackage(t *testing.T) {
	dir := t.TempDir()
	...
	write("thing.pb.go", "// Code generated by protoc-gen-go. DO NOT EDIT.\n\npackage daemon\n\ntype T struct{}\n")
	if holdsCensusableSource(dir) {
		t.Error("a directory of generated code was reported as censusable: the gate would fail every proto regeneration")
	}

	// The calibration must still catch a REAL mismatch, or fixing the above deletes it.
	write("hand_written.go", "package daemon\n\ntype U struct{}\n")
	if !holdsCensusableSource(dir) {
		t.Error("hand-written source alongside generated code was missed: a genuine root mismatch would go unnoticed")
	}
}
