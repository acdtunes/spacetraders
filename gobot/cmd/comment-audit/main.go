...
func registerFlags(fs *flag.FlagSet) *cliFlags {
	return &cliFlags{
		...
		tolerance:  fs.Float64("tolerance", 0, "comment lines forgiven per line of the package before a regression fires; 0 is strict, and strict is the default because such a budget grows with the package"),
		...
		gate:    fs.Bool("gate", false, "apply the standing gate policy: baseline regression plus the built-in per-file ceiling. Prefer this to spelling the limits out, so a caller cannot drift from the policy the gate test enforces. Refuses the limit flags it would otherwise override. This is an AD-HOC audit against the RECORDED census: TestCommentDensityGate anchors each touched package to main instead, and that test is the check of record — run `make comment-audit-check` for it"),
