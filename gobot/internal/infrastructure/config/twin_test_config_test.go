...
func TestTwinTestConfigIsolatesFromProduction(t *testing.T) {
	...
	checks := []struct{ name, got, want string }{
		...
		// daemon.address is NOT asserted: the knob was removed from DaemonConfig by
		// the config classification audit (e1687f12) as display-only. It was never
		// plumbed to a listener — at this branch's merge base its only readers were
		// `config show` and its own default — so it never was an isolation boundary.
		// The real boundaries are pid_file, socket_path, database.url and metrics.port.
		{"database.url", cfg.Database.URL, "postgresql://spacetraders:dev_password@localhost:5434/spacetraders_test?sslmode=disable"},
	}
