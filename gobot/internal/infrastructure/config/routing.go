...
type RoutingConfig struct {
	...
	// Resolved to an ABSOLUTE path at config load: empty → the config file's
	...
	// GateBackoff tunes the gate-graph negative-result backoff: how long an
	...
	// lookup treats it as stale and triggers a live re-fetch. The gate-graph
	...
	// SkipUnchartedGateFetch is the doomed-call precondition switch (default ON).
	...
	// restores the probe-then-backoff behaviour — is preserved.
	SkipUnchartedGateFetch *bool `mapstructure:"skip_uncharted_gate_fetch"`

	// ChartGateOnArrival is the chart-on-gate-arrival switch (default ON). A hull
