...
func (h *RunWorkerHandler) executeMining(
	...
	for {
		...
		if ship.IsCargoFull() {
			...
			cargoItems := make([]miningQueries.CargoItemValue, len(ship.Cargo().Inventory))
			for i, item := range ship.Cargo().Inventory {
				cargoItems[i] = miningQueries.CargoItemValue{
					Symbol: item.Symbol,
					Units:  item.Units,
			...
			if err != nil {
				...
				evalResult := evalResp.(*miningQueries.EvaluateCargoValueResponse)
