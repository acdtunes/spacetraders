...
// --- sp-pix3: chart BFS-outward from the charted frontier (ring by ring) ---

// sp-pix3 (i): the sweep charts BFS-OUTWARD from the charted frontier — with a limited dispatch
// budget it spends on the uncharted gate NEAREST an idle probe over stored adjacency, NOT
// whichever backlog system sorts first alphabetically. Three gate-uncharted market systems:
//
//	X1-AAA — behind an uncharted gate: NO stored-adjacency route from the probe → unroutable;
//	X1-BBB — routable but DEEP (8 jumps): a 5–9-jump relay that fail-closes at live jump time
//	         on an uncharted transit gate (the 596-failure SCOUT_REPOSITION storm);
//	X1-CCC — routable and NEAR (1 jump): a genuine frontier edge, reachable THIS tick.
//
// With budget 1 the sweep must spend it on the near X1-CCC — skipping the unroutable X1-AAA and
// DEFERRING the deep X1-BBB (it becomes reachable once a nearer ring charts). MUTATION GUARD:
// drop the nearest-first ranking (revert to the alphabetical backlog order) and the budget lands
// on X1-BBB — exactly the deep relay that strands on an uncharted transit gate — failing this.
func TestReconcileGateChartSweep_PrefersNearestRoutable_OverDeepAndUnroutable(t *testing.T) {
	clock := &shared.MockClock{CurrentTime: time.Now()}
	sats := []*navigation.Ship{newScoutTestSatellite(t, "SAT-1", "X1-HUB-A1")}
	gg := &fakeGateGraph{hops: map[string]int{
		// X1-AAA: no entry → unroutable (behind an as-yet-uncharted gate).
		"X1-HUB->X1-BBB": 8, // routable but deep — the fail-closing relay into the dark
		"X1-HUB->X1-CCC": 1, // routable and near — the genuine charted-frontier edge
	}} // no adjacency → AAA/BBB/CCC are all gate-uncharted backlog targets
	handler, daemonClient := gateReconcileTestHandler(t, clock, sats,
		map[string]float64{"X1-AAA": 1, "X1-BBB": 2, "X1-CCC": 3}, gg)

	idle := append([]*navigation.Ship(nil), sats...)
	handler.reconcileGateChartSweep(context.Background(), gateReconcileTestCmd(1), &idle)

	require.Len(t, daemonClient.persistedRepositionCmds, 1, "the single-relay budget spends on exactly one target")
	require.Equal(t, "X1-CCC-A1", daemonClient.persistedRepositionCmds[0].DestinationWaypoint,
		"the budget must go to the NEAREST routable uncharted gate (X1-CCC, 1 jump) — never the deep fail-closing relay (X1-BBB, 8 jumps) nor the unroutable X1-AAA behind an uncharted gate")
	require.Empty(t, idle, "the one dispatched probe is consumed from the idle pool")
}

// sp-pix3 (ii): the charted frontier expands one ring per cycle. X1-RING2 sits BEHIND X1-RING1's
// uncharted gate, so cycle 1 cannot route to it (no stored-adjacency path) and the sweep charts
// only the reachable X1-RING1, DEFERRING X1-RING2. Once X1-RING1's gate is charted — its edges
// enter stored adjacency (excluding it as a target) and X1-RING2 becomes routable THROUGH it —
// cycle 2 dispatches to the previously-unroutable X1-RING2. This is the monotonic outward
// expansion the fail-closed deep relays never achieved: never dispatch into the uncharted dark,
// just extend the boundary one reachable ring at a time.
func TestReconcileGateChartSweep_FrontierExpandsOutwardOverCycles(t *testing.T) {
	clock := &shared.MockClock{CurrentTime: time.Now()}
	gg := &fakeGateGraph{hops: map[string]int{
		"X1-HUB->X1-RING1": 1, // RING1 is one jump from the probe — reachable now
		// X1-RING2 has NO route from HUB yet — it is behind RING1's still-uncharted gate.
	}}
	handler, daemonClient := gateReconcileTestHandler(t, clock,
		[]*navigation.Ship{newScoutTestSatellite(t, "SAT-1", "X1-HUB-A1")},
		map[string]float64{"X1-RING1": 1, "X1-RING2": 2}, gg)

	// Cycle 1: only RING1 is routable; RING2 is deferred (behind an uncharted gate), never fired.
	idle1 := []*navigation.Ship{newScoutTestSatellite(t, "SAT-1", "X1-HUB-A1")}
	handler.reconcileGateChartSweep(context.Background(), gateReconcileTestCmd(5), &idle1)

	require.Len(t, daemonClient.persistedRepositionCmds, 1, "cycle 1 charts only the reachable near ring")
	require.Equal(t, "X1-RING1-A1", daemonClient.persistedRepositionCmds[0].DestinationWaypoint,
		"cycle 1 dispatches to the routable RING1 — never the unroutable RING2 behind it")

	// RING1's gate is now charted: it enters stored adjacency (so the enumeration excludes it as a
	// target) and RING2 becomes routable THROUGH it. A fresh idle probe stands ready.
	gg.adjacency = map[string][]system.GateEdge{"X1-RING1": {{ConnectedSystem: "X1-RING2"}}}
	gg.hops["X1-HUB->X1-RING2"] = 2

	// Cycle 2: RING2 is now reachable → the frontier expands outward to it.
	idle2 := []*navigation.Ship{newScoutTestSatellite(t, "SAT-2", "X1-HUB-A1")}
	handler.reconcileGateChartSweep(context.Background(), gateReconcileTestCmd(5), &idle2)

	require.Len(t, daemonClient.persistedRepositionCmds, 2, "cycle 2 adds exactly one more dispatch — the newly-reachable ring")
	require.Equal(t, "X1-RING2-A1", daemonClient.persistedRepositionCmds[1].DestinationWaypoint,
		"once RING1 charts, the previously-unroutable RING2 becomes dispatchable — the charted frontier expanded outward one ring")
}
