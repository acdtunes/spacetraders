...
// moment anything contacts it: sp-l0aqy cost ~290 API failures an hour for ten
// hours by building a work list from an unscoped read of exactly this table, while
// utilisation sat at 88% against an 85% ceiling. An implementation therefore scopes
// to the open era through persistence.OpenEraScope — NOT openEraID +
// eraScopePredicate, whose nil path matches `era_id IS NULL` and so answers from
// pre-backfill rows instead of refusing — and returns an ERROR when the era cannot
// be resolved. The pass reads that error as "dispatch nothing this tick".
//
