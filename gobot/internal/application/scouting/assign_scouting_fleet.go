...
// filterScoutShips filters ships to only probe/satellite types in the specified system
// Note: Since domain Ship doesn't expose frame type, we use the symbol pattern
// Probe ships typically have "PROBE" in their symbol, satellites have "SAT" or "SATELLITE"
func (h *AssignScoutingFleetHandler) filterScoutShips(ships []*navigation.Ship, systemSymbol string) []*navigation.Ship {
	...
	for _, ship := range ships {
		...
		// Filter by symbol pattern (probe/satellite)
		symbol := strings.ToUpper(ship.ShipSymbol())
		if strings.Contains(symbol, "PROBE") || strings.Contains(symbol, "SAT") {
