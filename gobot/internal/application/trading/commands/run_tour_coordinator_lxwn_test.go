...
// freshListings must drop rows past marketDataAgeFloor so the pre-rank scores only markets the
...
func TestReposition_FreshListings_ExcludesStaleBestLane(t *testing.T) {
	now := time.Now()
	stale := now.Add(-2 * marketDataAgeFloor) // well past the 75-min snapshot cap
	...
	kept := freshListings(listings, now, marketDataAgeFloor)
	if len(kept) != 2 {
	...
	if len(freshListings(unstamped, now, marketDataAgeFloor)) != 1 {
