...
//     maxListingAge fresh-lane filter, the stale-lane counter, and the capped-spread pre-rank score.
...
func (h *RunTourCoordinatorHandler) relocationRegionFreshness(ctx context.Context, cmd *RunTourCoordinatorCommand, system string, now time.Time) (time.Duration, string, bool) {
	...
	// The SAME maxListingAge pre-filter scoreRepositionNeighbors applied when it built the candidate,
