...
// trade-route coordinator's exported RepositionToWaypoint, which delegates to the SAME
// multi-jump travel() the arb/trade circuits use — the ferry reuses that machinery
// rather than re-implementing jump logic (RULINGS: reuse). Narrowing the dependency to
// this one method keeps the worker testable with a tiny fake and states exactly what it
// touches: movement, nothing else. (Twin of the scouting package's Repositioner; the
// trading/commands package has no Repositioner interface of its own — only the method on
// RunTradeRouteCoordinatorHandler — so it is declared here, mirroring scout_reposition.)
