...
func TestPurchaseFabricatedOutput_GateNode_DrainsToAvailableSupply_NoRateCap(t *testing.T) {
	...
	buy := func(lot int) int {
		units, _, err := executor.purchaseFabricatedOutput(ctx, dockRaceGood, gateFillTestFactoryWP, dockRaceShip, shared.MustNewPlayerID(1), tradeVolume)
		if err != nil {
...
func TestPurchaseFabricatedOutput_GateNode_CargoBoundsTheBuy(t *testing.T) {
	...
	units, _, err := executor.purchaseFabricatedOutput(ctx, dockRaceGood, gateFillTestFactoryWP, dockRaceShip, shared.MustNewPlayerID(1), 43)
	if err != nil {
