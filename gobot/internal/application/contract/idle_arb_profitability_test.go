...
// candidate lane per pass. The gate re-prices EVERY pass from the freshly-read
// ask/bid (never a cached spread): net_per_u = (bid - ask) - fuel_per_u, launch
// ONLY IF net_per_u >= max(MinNetProfitPerUnit, ceil(NetProfitFraction × ask)).
// A lane the fleet inflated below the floor is skipped that pass and
// AUTO-RE-ENTERS the next pass its price recovers — a skip never marks the lane
// mutex, so re-entry is not blocked by this dispatcher's own prior refusal.
...
// A thin lane the fleet's own buying has inflated is STOPPED before it goes
// net-negative. Modeled as two snapshots of the same
...
// The sell-side mirror: a lane the fleet's own dumps have DECAYED — the sink bid
// walked down while the buy held — is stopped the same way, before it goes
// net-negative. Same net_per_u = bid − ask − fuel comparison, driven from the
// sink side. Proves the guard catches both self-inflation (ask up) and dump
// decay (bid down).
...
// Self-regulating: a lane skipped for want of net margin AUTO-RE-ENTERS the pass
// its price recovers. Because a profitability skip never marks the lane mutex,
// the recovered lane is free to fly — the dispatcher's own prior refusal never
// latches it off.
...
// The RELATIVE floor: net >= 20% of the buy price is what stops a
...
// position: a lane whose per-unit BUY outlay is large — a big treasury dip
// between the buy and the sell — still flies when its net round-trip margin
// clears the floor. A gate that alarmed on the buy-leg outlay would wrongly
// refuse this profitable leg.
...
// summary (message text — the CLI drops metadata maps), so it is legible where
// the analyst scan is diffed.
