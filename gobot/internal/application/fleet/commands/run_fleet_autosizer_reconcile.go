...
func (h *RunFleetAutosizerCoordinatorHandler) reconcileOnce(ctx context.Context, cmd *RunFleetAutosizerCoordinatorCommand) (reconcileResult, error) {
	...
	// read below this line — the shared tick inputs, each class's demand, and (inside sizeClass)
	// the shipyard price walk. That last one is the expensive one and it is why the switch cannot
	// live further down.
	//
