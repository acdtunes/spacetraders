...
// a desired-state table of per-system market-freshness assignments
...
//
// The "scan queue" is exactly the set of unmanned sweep-once posts — the captain
// seeds them from the frontier census, so no separate queue subsystem is needed.
...
// ExtraSlots entries are addressed by their 0-based index.
const primarySlotIndex = -1

// ScoutPostSlot is one manning slot BEYOND the primary of a multi-probe post:
// an additional probe touring a DISJOINT partition of the same
...
// below — and its primary probe tours ALL the system's markets. A multi-hull post
// additionally carries ExtraSlots and a per-slot frozen Partition.
//
...
//
// RepositionContainerID tracks an in-flight cross-gate RELAY that is jump-routing
// an idle satellite from another system TO this post's primary slot. It is mutually
...
type ScoutPost struct {
	...
	// Hulls is the probe budget N: the post is toured by N probes over N DISJOINT
	// market partitions, so freshness per market ≈ (M/N markets) × circuit pace. 0 or
	// 1 ⇒ single-hull. Only standing posts partition; sweep-once is always single-hull
	...
	// ExtraSlots holds slots 1..N-1 of a multi-hull post. Empty for a
	...
	// dead tour without the tour surviving to be observed healthy. The
	...
	// tour again after the respawn cap was hit. Zero ⇒ not parked. While it
...
// one per ExtraSlots entry. A single-hull post yields exactly one handle (the
// primary). The count can be less than HullBudget() before the partitioner has
// materialized the extra slots; the reconciler's partition pass grows ExtraSlots to
// HullBudget()-1.
...
// slots. Order is primary-first, then extra slots in index order.
...
// MannedCount is how many of the post's slots currently have a hull.
...
// hull — the multi-hull generalization of "manned". A single-hull post
...
// index == primarySlotIndex addresses the PRIMARY slot (the post's scalar fields +
// PrimaryPartition); index >= 0 addresses ExtraSlots[index]. Reads and writes go
// THROUGH to the backing store, so a single-hull post (only the primary slot)
// persists with just the scalar fields.
...
// so a single-hull post keeps the un-suffixed key.
...
// Partition returns this slot's frozen disjoint market tour. Empty ⇒ the
...
// satellite is still crossing gates. Retained for the single-hull callers;
...
// satellite toward this post's PRIMARY slot. Retained for the single-hull callers;
// multi-hull reconcile logic works per ScoutSlotRef instead.
