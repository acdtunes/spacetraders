...
// multiplePct/100 times the fleet's intra-system margin per tranche while paying its gate no more
// than maxFeeSharePct of the larger of that margin and fleetVisitMargin. No baseline, no lane.
func IsFatLane(marginPerTranche, transitSeconds, fleetCreditsPerSec float64, gateFee int64, intraMarginPerTranche, fleetVisitMargin float64, multiplePct, maxFeeSharePct int) bool {
	if intraMarginPerTranche <= 0 {
		return false
	}
	if float64(gateFee) > float64(maxFeeSharePct)/100*math.Max(marginPerTranche, fleetVisitMargin) {
		return false
	}
