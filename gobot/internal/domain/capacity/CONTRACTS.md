...
- `TopologySignals{Clusters []ClusterState, IdleHulls []HullUtilization}`; `ClusterState{HubSymbol, Warehouses, Stockers, Workers}`; `WarehouseState{ShipSymbol, Waypoint, Buffer []BufferedStock, GoodCaps map[string]int}`; `BufferedStock{Good, Units}`; `StockerState`/`WorkerState{ShipSymbol, Waypoint}` — the ACTUAL topology DIFF compares against.
- **`TopologySignals.IdleHulls` (ADDITIVE, st-zr0 — st-7ee MUST fill)**: the reuse-eligible idle subset of `Utilization.Hulls` (`Idle == true`), same tick, same snapshot (the sibling rule to the `FleetHullCount` consistency note below). `Differ.Diff` receives ONLY `TopologySignals`, so this is the ONLY channel through which the tier-1 reuse-first supply reaches the escalation ladder. The differ re-verifies eligibility per hull (idle, in the contract reuse pool — undedicated or the contract op's own "contract" reserve, never the command frigate — `ReuseEligibleFleet`, and not already serving a cluster role) — an over-filled slice fails SAFE (ineligible entries are skipped, never reassigned) — but an EMPTY slice silently starves tier 1: every hull gap then escalates straight to tier-4 capital proposals even when free idle hulls exist. Filling it is part of the SENSE lane's definition of done.
...
- `Action{Tier, Verb, HubSymbol, ShipSymbol, Good, UnitsCap, TargetWaypoint, EstimatedCostCredits, HullDelta, ProjectedPerHullCrHr, GapKind, WarehouseDelta, StockerDelta, WorkerDelta, Count, Reason}` — one convergence step. `HullDelta` (st-zr0 fills; pure topology arithmetic: `buy_hull`=1, `add_cluster`=its warehouse+stocker+worker counts, 0 for free tiers) is the governor's ROI-arithmetic input `d`.
- **Machine-readable routing fields (ADDITIVE, st-zr0 fills — the actuator st-5ig reads these; NEVER parse `Reason` prose)**: `GapKind` classifies the gap the action closes and carries the hull ROLE for `reassign_hull`/`buy_hull` (`GapWarehouseShort`/`GapStockerShort`/`GapWorkerShort` ⇒ warehouse/stocker/worker hull); `WarehouseDelta`/`StockerDelta`/`WorkerDelta` decompose `HullDelta` per role (`add_cluster` carries its composition, `buy_hull` stamps 1 on the bought role; sum == `HullDelta`); `Count` is `rebalance_workers`' moved-worker quantity (0 for other verbs).
...
- `Differ.Diff(ctx, desired, actual TopologySignals, Calibration) ([]Action, error)` — cheapest-lever-first order; empty desired ⇒ zero actions. The spec's tier-3 gate ("buffer adjust gated if it forces a NEW stocker") is implemented as WITHHOLDING inside DIFF: a hub whose desired `StockerCount` exceeds its actual stockers emits NO demand-expanding buffer adjustments (whitelist adds, cap raises) — they self-heal after the (tier-4-gated where capital) stocker capacity lands; demand-shedding adjustments (cap reductions, de-whitelists) always flow. Worker shortfalls on covered AND uncovered hubs draw the fleet's own worker surplus (one `rebalance_workers` per shortfall hub) before any capital worker is proposed — `add_cluster`'s `HullDelta` excludes surplus-coverable workers.
