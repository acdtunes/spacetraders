...
// Multi-hop trade-tour planning types. These are the Go-side
...
// start system) — the per-tour length lever. 0 means "unset":
...
// without a redeploy.
//
// Closed + AnchorSystem select closure mode: Closed=true makes
...
// assembles for the planner: a good the contract history
...
// planner to NET out of available tranche depth. The daemon decays the
...
type TourPlan struct {
	...
	// tour selection so pure-liquidation tours remain plannable; this field is
	// reporting-only, letting the planned-manifest log
	...
	// from haul-to-storage DEPOSIT legs: sum of units*SyntheticBid
