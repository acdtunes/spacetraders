...
func TestClaimShip_RejectsForeignOperationOnWarehouseHull(t *testing.T) {
	...
	require.NoError(t, db.Create(&persistence.ShipModel{
		ShipSymbol:       "WAREHOUSE-HULL-7",
		PlayerID:         playerID.Value(),
		AssignmentStatus: "idle",
		DedicatedFleet:   "warehouse",
	}).Error)

	err := repo.ClaimShip(context.Background(), "WAREHOUSE-HULL-7", "mfg-worker-1", playerID, "manufacturing")
	require.Error(t, err)
	...
	require.NoError(t, db.Where("ship_symbol = ?", "WAREHOUSE-HULL-7").First(&model).Error)
...
func TestClaimShip_WarehouseOperationClaimsItsDedicatedHull(t *testing.T) {
	...
	err := repo.ClaimShip(context.Background(), "WAREHOUSE-HULL-7", "warehouse-X1-HOME-A1", playerID, "warehouse")
