...
// wins, our stale +10 would have clobbered the +400.
...
func TestSaveWithRetry_ExhaustsThenFallsBackToLastWriteWins(t *testing.T) {
	repo, db, pid := newShipWriteTestRepo(t)
	repo.SetCASRetryPolicy(1) // 1 retry → attempts 0 and 1, then fallback
