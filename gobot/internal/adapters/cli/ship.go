...
Examples:
  ...
  spacetraders ship refuel --ship ENDURANCE-1 --agent ENDURANCE`,
	...
	cmd.AddCommand(newShipUnreserveCommand())
...
// resolvePlayerPointers converts the resolved player identifier into the optional
// pointer form expected by the daemon client.
func resolvePlayerPointers() (*int32, *string, error) {
	playerIdent, err := resolvePlayerIdentifier()
	if err != nil {
		return nil, nil, err
	}
	...
	return playerID, agentSymbol, nil
}
...
func newShipReserveCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "reserve",
		Short: "Reserve a ship so coordinators do not auto-claim it",
		Long: `Mark a ship as reserved. A reserved hauler is held out of every
coordinator's dynamically-discovered idle-hauler pool, so a ship dedicated to
the jump-gate or manufacturing stream is never auto-claimed by the contract
coordinator. Clear the reservation with 'ship unreserve'.

Examples:
  spacetraders ship reserve --ship TORWIND-4 --reason "gate hauler"
  spacetraders ship reserve --ship TORWIND-4 --agent TORWIND`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if shipSymbol == "" {
				return fmt.Errorf("--ship flag is required")
			}
			...
			playerID, agentSymbol, err := resolvePlayerPointers()
			if err != nil {
				return err
			}
			...
			response, err := client.SetShipReservation(ctx, shipSymbol, true, reasonPtr, playerID, agentSymbol)
			if err != nil {
				return fmt.Errorf("failed to reserve ship: %w", err)
			}

			fmt.Printf("✓ Ship %s reserved\n", response.ShipSymbol)
			if response.Reason != "" {
				fmt.Printf("  Reason: %s\n", response.Reason)
			}
			return nil
	...
	cmd.Flags().StringVar(&shipSymbol, "ship", "", "Ship symbol (required)")
	cmd.Flags().StringVar(&reason, "reason", "", "Optional reason for the reservation")

	return cmd
}

// newShipUnreserveCommand creates the ship unreserve subcommand
func newShipUnreserveCommand() *cobra.Command {
	var shipSymbol string

	cmd := &cobra.Command{
		Use:   "unreserve",
		Short: "Clear a ship's reservation, returning it to the auto-claim pool",
		Long: `Clear a ship's reservation. The ship returns to every coordinator's
idle-hauler pool and can once again be auto-claimed (e.g. by the contract
coordinator).

Examples:
  spacetraders ship unreserve --ship TORWIND-4
  spacetraders ship unreserve --ship TORWIND-4 --agent TORWIND`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if shipSymbol == "" {
				return fmt.Errorf("--ship flag is required")
			}
			...
			response, err := client.SetShipReservation(ctx, shipSymbol, false, nil, playerID, agentSymbol)
			if err != nil {
				return fmt.Errorf("failed to unreserve ship: %w", err)
			}

			fmt.Printf("✓ Ship %s unreserved\n", response.ShipSymbol)
			return nil
		},
	}
