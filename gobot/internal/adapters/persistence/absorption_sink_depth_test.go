...
// of tranches. The sink's LISTING BREADTH is what separates them: the micro-market that
// one tranche really does take off the board lists a single good, the hub that absorbs
// the same tranche without a measurable bid move lists many.
const (
...
func replaySinkAfterOneTranche(t *testing.T, listings int) bool {
	t.Helper()
...
// The deep-hub lane stays on the board: a multi-listing sink takes the same tranche
...
func TestAbsorptionSinkDepth_DeepHubLaneAdmittedAfterATranche(t *testing.T) {
	...
	require.True(t, replaySinkAfterOneTranche(t, hubListings),
...
func TestAbsorptionSinkDepth_ThinMicroMarketRefusedAfterItsTranche(t *testing.T) {
	for _, listings := range []int{1, 2} {
		t.Run(fmt.Sprintf("%d listing(s)", listings), func(t *testing.T) {
			require.False(t, replaySinkAfterOneTranche(t, listings),
				"a micro-market that one tranche takes off the board stays embargoed")
		})
	}
}
...
func TestAbsorptionSinkDepth_UnreadableBreadthRefuses(t *testing.T) {
	require.False(t, replaySinkAfterOneTranche(t, 0),
