...
func TestMarketRepo_FindBestMarketForBuying_ScopedToPlayer(t *testing.T) {
	...
	// My only market for the good: the worst possible score.
	require.NoError(t, db.Create(&persistence.MarketData{
		WaypointSymbol: "X1-BUY-M1", GoodSymbol: "CLOTHING",
		PurchasePrice: 400, SellPrice: 500, TradeVolume: 10,
	...
	require.NoError(t, db.Create(&persistence.MarketData{
		...
		PurchasePrice: 5, SellPrice: 10, TradeVolume: 100,
	...
	require.Equal(t, 500, best.SellPrice)
}
