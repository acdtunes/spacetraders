...
// without releasing it. A container is LIVE while non-terminal (PENDING / RUNNING /
// STOPPING / INTERRUPTED) and dead once terminal (COMPLETED / FAILED / STOPPED); a
// container that has vanished from the table entirely is likewise absent from the live
// set. It reads the same containers table the daemon's own recovery consults, so "live"
// here means exactly what the daemon runtime means.
