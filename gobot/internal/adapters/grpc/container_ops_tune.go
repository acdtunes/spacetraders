...
func tunableKnobsByContainerType() map[string]map[string]TuneBound {
	...
	return map[string]map[string]TuneBound{
		...
		string(container.ContainerTypeFrontierExpansion): {
			...
			"max_probe_price":            {Type: "int", Min: 0, Max: 5_000_000, Default: frontier["max_probe_price"], Unit: "credits", Description: "sp-3u5d per-unit probe price ceiling: DEFER the buy when the final chosen quote (after sibling-spread) exceeds this — the backstop for reachability-bound deep yards with no cheaper sibling (price spirals to 210-235k); the post stays dark and retries next cycle. 0 = disabled (buys at any price)"},
			...
			// sp-6vep reuse-before-buy the deep frontier. ALL DEFAULT-SAFE (0 ⇒ today's buy-only path, byte-identical); next era arms them.
			"probe_reuse_enabled":        {Type: "int", Min: 0, Max: 1, Default: frontier["probe_reuse_enabled"], Unit: "flag", Description: "sp-6vep reuse-before-buy master switch: 1 ⇒ try RELAYING an existing edge probe onto a target virgin before buying (fixes the deep-frontier deadlock where no yard/buyer is reachable), 0 (default) ⇒ today's buy-only path. Arm ALONGSIDE reuse_value_ceiling>0 (else reuse borrows off no system)"},
			"edge_relay_max_hops":        {Type: "int", Min: 1, Max: 12, Default: frontier["edge_relay_max_hops"], Unit: "hops", Description: "sp-6vep max gate-hops to relay an EXISTING probe to a post — reach measured FROM the nearest edge probe TO the target (~1 for the queued virgins), replacing the fixed 5-jump-from-buyer wall. Inert while probe_reuse_enabled=0"},
			"reuse_value_ceiling":        {Type: "int", Min: 0, Max: 5_000_000, Default: frontier["reuse_value_ceiling"], Unit: "credits", Description: "sp-6vep depth-vs-freshness judgment: reuse BORROWS a probe only off a system whose trade-value is BELOW this — cannibalize low-value deep-edge freshness to fund exploration, NEVER strip a probe off a high-value core market. 0 (default) ⇒ borrow off NO system (reuse a safe no-op until armed)"},
			"snowball_neighbors":         {Type: "int", Min: 0, Max: 1, Default: frontier["snowball_neighbors"], Unit: "flag", Description: "sp-6vep walk-outward: 1 ⇒ after a relayed probe charts a virgin S, auto-declare posts for S's uncharted gate-neighbors so one probe walks the frontier outward; 0 (default) ⇒ one head per cycle"},
			"post_inflight_timeout_secs": {Type: "int", Min: 60, Max: 86_400, Default: frontier["post_inflight_timeout_secs"], Unit: "seconds", Description: "sp-6vep NO-TIMEOUT deadlock fix (independent of reuse): abandon an unmanned, relay-free in-flight sweep-once post older than this so the in-flight cap never jams permanently at 5/5. 0 (default) ⇒ DISABLED, no post ever reaped (byte-identical to today); arm next era to a generous window (e.g. 1800)"},
		},
		...
		string(container.ContainerTypeBootstrapCoordinator): {
			...
			// SCALED contract op (haulers + a SUSTAINED rolling-window $/hr) instead of the bare instantaneous
			// income_bar (which one contract payout trivially cleared — the ktio deadlock). 0 (default) ⇒
			// byte-identical. MUST arm together with ktio-B (autosizer-early) or the arc wedges in INCOME. The
			// two bars are phase thresholds like income_bar, NOT money-floors (RULINGS #5).
			"scaled_gate_entry":       {Type: "int", Min: 0, Max: 1, Default: bootstrap["scaled_gate_entry"], Unit: "flag", Description: "sp-fp3y: 1 ⇒ GATE entry requires a SCALED contract op — haulers ≥ gate_min_haulers AND a SUSTAINED (rolling-window) $/hr ≥ gate_income_bar — instead of instantaneous income_bar; 0 (default) ⇒ byte-identical. Arm WITH ktio-B (autosizer-early)"},
			...
			"autosizer_early_scaling": {Type: "int", Min: 0, Max: 1, Default: bootstrap["autosizer_early_scaling"], Unit: "flag", Description: "sp-sjvv (ktio-B): 1 ⇒ bootstrap launches the fleet autosizer EARLY during the DATA/INCOME scaling window (so the capacity reconciler's contract-delivery demand has a buyer) AND defers its own contract-hauler buys to it (single-buyer arbitration); 0 (default) ⇒ the autosizer stays off the whole bootstrap run and bootstrap buys its haulers itself (byte-identical). ARM needs the ktio-A treasury floor (sp-bpdf) + the autosizer's own contract_delivery_hulls_enabled (sp-nkqn)"},
		},
	}
}
