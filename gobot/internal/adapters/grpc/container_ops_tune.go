...
func tunableKnobsByContainerType() map[string]map[string]TuneBound {
	...
	return map[string]map[string]TuneBound{
		...
		string(container.ContainerTypeBootstrapCoordinator): {
			...
			// never latches on a lightly-scaled op with no war chest and then cannibalizes the contract op below
			// the depot-staging floor. These floors are phase thresholds, not money-guards (RULINGS #5).
			...
			"gate_contract_floor":           {Type: "int", Min: 0, Max: 50, Default: bootstrap["gate_contract_floor"], Unit: "hulls", Description: "contract-earner floor held through GATE — GATE repurposes only the surplus ABOVE this to construction, never below (the reconciler withholds the staging depot below a 2-hauler pool). Default 2, supersedes min_contract_earners"},
