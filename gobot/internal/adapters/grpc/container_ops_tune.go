...
func tunableKnobsByContainerType() map[string]map[string]TuneBound {
	...
	return map[string]map[string]TuneBound{
		...
		string(container.ContainerTypeProbeSensingCoordinator): {
			...
			"chart_hull_cap":  {Type: "int", Min: 1, Max: 5, Default: sensing["chart_hull_cap"], Unit: "hulls", Applies: TuneAppliesLive, Description: "how many probes ONE dark system may be charted by at once, each working a disjoint angular share of its uncharted waypoints. A charting tour is serial — one waypoint per turn per hull — so a system's completion time is its outstanding count divided by its crew, and this is what bounds the crew. THE MAXIMUM IS DERIVED, NOT PICKED, and that is why it is 5 rather than a round number: every hull past the first pays its own walk to the system (two seed steps per gate hop, plus the arrival sweep) before it charts anything, so the c-th hull only pays for itself while outstanding/c clears the measured walk to rank c — 19 outstanding for the 2nd, 32 for the 3rd, 46 for the 4th, 63 for the 5th, 81 for the 6th. The largest chartable system on this map holds 72 waypoints, so no system that exists can pay for a 6th hull and the bound stops there. 1 IS THE OFF SWITCH: one hull per system over the whole list, whatever the thresholds say. NOT a money guard — a crew hull is a probe like any other and still passes the buy floor and probe_cap. Applies next tick"},
			"chart_hull_2_at": {Type: "int", Min: 1, Max: 500, Default: sensing["chart_hull_2_at"], Unit: "waypoints", Applies: TuneAppliesLive, Description: "outstanding uncharted waypoints at which a system earns its SECOND charting hull. Under it a system keeps the one hull it has, so a nearly-finished system never draws a fresh walk for the handful of stops left. The default is that hull's measured break-even against its own walk (see chart_hull_cap). Raise it to spread the fleet across more systems, lower it to finish the ones it is already on sooner; set it above any real system's count to hold every system to one hull. Applies next tick"},
			"chart_hull_3_at": {Type: "int", Min: 1, Max: 500, Default: sensing["chart_hull_3_at"], Unit: "waypoints", Applies: TuneAppliesLive, Description: "outstanding uncharted waypoints at which a system earns its THIRD charting hull, on the same terms as chart_hull_2_at and bounded by chart_hull_cap regardless. IT ALSO SETS THE STEP BEYOND THE THIRD: every further chart_hull_3_at-minus-chart_hull_2_at waypoints of outstanding work earn one more hull, so the crew grows with the size of the system rather than flattening at three, and the pair moves the whole ladder together instead of leaving a hidden constant to disagree with them. Thresholds set out of order supply no step and the documented one is used. Applies next tick"},
