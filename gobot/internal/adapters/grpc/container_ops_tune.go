...
func tunableKnobsByContainerType() map[string]map[string]TuneBound {
	...
	return map[string]map[string]TuneBound{
		...
		string(container.ContainerTypeProbeSensingCoordinator): {
			...
			"surge_inflight_cap":         {Type: "int", Min: 1, Max: 200, Default: sensing["surge_inflight_cap"], Unit: "hulls", Applies: TuneAppliesLive, Description: "SENSING SURGE: how many surplus probes may be FLYING toward charted-but-unpriced systems at once. A standing concurrency bound, not a per-tick burst — each tick tops the in-flight population up to it and stops, so the surge's cost cannot grow tick over tick. IT IS A BASE, NOT A CEILING: like every other paced sensing pass it is scaled by the idle share of the request budget (expansion_headroom_multiple, the same linear term the placement budget takes), so this number is the standing population at FULL saturation and the live cap is that number or more — read cap and cap_base on parked_sensing_surged for the pair. It is sized off API headroom rather than off the placement budget because a hull in transit costs the placement machine nothing until it reaches a gate, so that budget never bound this at any reachable size; what still bounds the flights' API cost is the placement budget itself, which is paced by the same reading and rotates least-recently-attempted-first. THE MAXIMUM IS A BACKSTOP, NOT THE SIZING RULE — it is set at the largest per-tick placement budget the engine can ever be given, above anything the fleet's surplus hulls or the unpriced pool can actually produce, so what answers in practice is how many idle probes exist and how many systems are left to price. Buying is untouched: the surge only ever dispatches hulls already paid for, and it never takes a probe standing a sensing watch. Not an off switch — 0 reverts to the default, and the pacing only ever raises it. Applies next tick"},
			...
			"chart_hull_cap":  {Type: "int", Min: 1, Max: 100, Default: sensing["chart_hull_cap"], Unit: "hulls", Applies: TuneAppliesLive, Description: "a HARD CEILING on how many probes ONE dark system may be charted by at once, each working a disjoint angular share of its uncharted waypoints. IT DOES NOT SIZE THE CREW — the walk does, measured every tick: a charting tour is serial, so a crew of c clears the outstanding count in 2*outstanding/c turns while the c-th hull first pays two turns per gate hop from the nearest system we hold plus its arrival sweep, and it is worth adding while outstanding/c clears that walk plus a half. A one-hop walk earns nine hulls on a twenty-waypoint system and a thirteen-hop walk earns one, so the answer moves with the map rather than sitting in a constant fitted to a map that has since changed (sp-glyoe). THE MAXIMUM IS DERIVED, NOT PICKED: it is set ABOVE THE LARGEST SYSTEM MEASURED so that the walk and the assembly bound are what answer and this only ever catches a number no real system can produce. It was 15, fitted to a 57-waypoint map, and on era-11's map that was the binding constraint instead of a backstop — systems average 54.3 waypoints and the largest holds 92, so a full crew covered a quarter of a system and the rest waited for later rounds (sp-vpf8t). THE ASSEMBLY BOUND STILL BINDS FIRST at every real size: hulls arrive one per system per tick, so rank c is granted after the ranks before it have already taken c*(c-1)/2 of the system's 2*outstanding turns, which exhausts the work at 19 hulls on a 92-waypoint system and at 15 on the era-11 average. RAISE OR LOWER IT to bound what the walk can buy; 1 IS THE OFF SWITCH, one hull per system over the whole list whatever else says. NOT a money guard — a crew hull is a probe like any other and still passes the buy floor and probe_cap. Applies next tick"},
			...
			"expansion_headroom_multiple": {Type: "int", Min: 1, Max: 20, Default: sensing["expansion_headroom_multiple"], Unit: "multiple", Applies: TuneAppliesLive, Description: "how many times its ceiling-era per-tick budget each PACED sensing pass may spend when the shared request budget is idle, scaled linearly to exactly the ceiling-era budget at full saturation: base*(1000+(m-1)*(1000-saturation_permille))/1000, floored at the base. It moves six burst budgets together — live jump-gate reads, expansion actions (seed steps and seed requests), placement moves, shipyard-catalogue reads, yard-presence dispatches and ledger reap releases — because they are one question asked six times: how big a burst may a tick fire when nothing is queued behind it. AT THE DEFAULT AND A 6% SATURATION the gate read goes from 3 reads a tick to 17, about 2,000 systems an hour of discovery against 360, which is what charts a ~9,900-system map inside one era. 1 IS THE OFF SWITCH and restores the pre-scaling pacing exactly; `tune expansion_headroom_multiple 0` reverts to the default, so it cannot be disarmed by zero. IT CAN ONLY EVER RAISE A BUDGET — no setting takes a pass below the constant it shipped with. NOT a money guard and no guard reads it: the buy floor, the probe cap and every spend gate are untouched at any value, and the reach bound MaxWalkRings is deliberately outside it (a foothold's cost is coverage, not calls). Which budget is actually binding is on the cycle line as budgets=<pass> used/limit and in parked_sensing_pass_budget_used/_limit. Applies next tick"},
