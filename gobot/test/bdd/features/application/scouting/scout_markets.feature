Feature: Scout Markets Command
  ...
  Scenario: Single ship assigns all markets
    ...
    When I execute scout markets command for player 1 with ships ["SHIP-1"] and markets ["X1-A1-MARKET", "X1-A2-MARKET"] in system "X1-A1" with 5 iterations
  ...
  Scenario: Multiple ships use VRP optimization
    ...
    When I execute scout markets command for player 1 with ships ["SHIP-1", "SHIP-2"] and markets ["X1-A1-MARKET", "X1-A2-MARKET"] in system "X1-A1" with 10 iterations
    Then the command should succeed
  ...
  Scenario: Container reuse (idempotent)
    ...
    When I execute scout markets command for player 1 with ships ["SHIP-1"] and markets ["X1-A1-MARKET"] in system "X1-A1" with 5 iterations
    Then the command should succeed
  ...
  Scenario: Ship not found
    Given a player with ID 1 and token "test-token" exists in the database
    When I execute scout markets command for player 1 with ships ["NONEXISTENT"] and markets ["X1-A1-MARKET"] in system "X1-A1" with 5 iterations
