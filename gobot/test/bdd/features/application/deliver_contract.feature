Feature: Deliver Contract Command
  ...
  Scenario: Deliver cargo for valid delivery
    ...
    Then the deliver command should succeed
  ...
  Scenario: Cannot deliver more than required
    ...
    Then the deliver command should fail with error "units exceed required"
  ...
  Scenario: Cannot deliver invalid trade symbol
    ...
    Then the deliver command should fail with error "trade symbol not in contract"
  ...
  Scenario: Cannot deliver when contract not found
    ...
    Then the deliver command should fail with error "contract not found"

  Scenario: Cannot deliver when player not found
    Given an accepted contract "CONTRACT-1" for player 999 with delivery of 100 "IRON_ORE" to "X1-GZ7-A1"
    And a ship "SHIP-1" owned by player 999 at waypoint "X1-GZ7-A1" with 50 "IRON_ORE" in cargo
    When I send DeliverContractCommand with contract "CONTRACT-1", ship "SHIP-1", trade "IRON_ORE", 50 units, player 999
    Then the deliver command should fail with error "player not found"
