Feature: Ship Entity
  ...
  # ============================================================================
  # Ship Helper Method Tests
  # ============================================================================

  Scenario: String representation includes ship symbol and location
    Given a docked ship at waypoint "X1-PORT" with 50/100 fuel
    When I get the ship string representation
    Then the ship string should contain "SHIP-1"
    And the ship string should contain "X1-PORT"

  Scenario: IsCargoEmpty returns true for empty cargo
    Given a ship at waypoint "X1-A1" with empty cargo
    When I check if cargo is empty
    Then cargo should be empty

  Scenario: IsCargoEmpty returns false for non-empty cargo
    Given a ship at waypoint "X1-A1" with 10 units of "IRON_ORE"
    When I check if cargo is empty
    Then cargo should not be empty

  Scenario: IsCargoFull returns true for full cargo
    Given a ship at waypoint "X1-A1" with cargo capacity 100
    And the ship has 100 units of "IRON_ORE"
    When I check if cargo is full
    Then cargo should be full

  Scenario: IsCargoFull returns false for partial cargo
    Given a ship at waypoint "X1-A1" with 50 units of "IRON_ORE" and capacity 100
    When I check if cargo is full
    Then cargo should not be full

  Scenario: HasCargoSpace returns true with available space
    Given a ship at waypoint "X1-A1" with 50 units of "IRON_ORE" and capacity 100
    When I check if ship has cargo space for 30 units
    Then ship should have cargo space

  Scenario: HasCargoSpace returns false without sufficient space
    Given a ship at waypoint "X1-A1" with 95 units of "IRON_ORE" and capacity 100
    When I check if ship has cargo space for 10 units
    Then ship should not have cargo space

  Scenario: AvailableCargoSpace calculates correctly
    Given a ship at waypoint "X1-A1" with 60 units of "IRON_ORE" and capacity 120
    When I get available cargo space
    Then available cargo space should be 60
