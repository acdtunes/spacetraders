...
// PersistManufacturingTaskWorkerContainer creates but does NOT start a manufacturing task worker container
func (m *MockDaemonClient) PersistManufacturingTaskWorkerContainer(ctx context.Context, containerID string, playerID uint, command interface{}) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	m.containers = append(m.containers, Container{
		...
		PlayerID: int(playerID),
		Status:   "PENDING",
		Type:     "manufacturing-task-worker",
	})
...
func (m *MockDaemonClient) StartManufacturingTaskWorkerContainer(ctx context.Context, containerID string, completionCallback chan<- string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
