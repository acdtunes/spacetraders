...
export const botToolDefinitions: Tool[] = [
  // ==================== CONTAINER MANAGEMENT ====================
  {
    ...
    inputSchema: {
      ...
      properties: {
        player_id: {
          type: "integer",
          description: "Optional: Filter containers by player ID"
        }
      }
    }
  },
  ...
  {
    name: "ship_list",
    description: "List all ships for a player. Shows ship symbol, location, navigation status, fuel levels, cargo capacity, and engine speed.",
    inputSchema: {
      type: "object",
      properties: {
        player_id: {
  ...
  {
    name: "ship_info",
    description: "Get detailed ship information including location, navigation status, fuel levels, cargo inventory with items, and engine speed.",
    inputSchema: {
      type: "object",
      properties: {
        ship: {
          type: "string",
          description: "Ship symbol to retrieve details for (e.g., CHROMESAMURAI-1)"
        },
        player_id: {
          type: "integer",
          description: "Player ID (optional if agent specified)"
