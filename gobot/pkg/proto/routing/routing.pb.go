...
type TourConstraints struct {
	...
	// sp-9idvn: per-DEPARTURE-GATE jump fee, learned Go-side from the ledger's own
	// recorded jumps. The solver prices a crossing's first hop from the departure
...
func (x *TourConstraints) GetGateFees() []*GateFee {
	if x != nil {
		return x.GateFees
	}
	return nil
...
//
// KEYED ON ONE SYSTEM, UNLIKE InterSystemHopDistance, and the difference is measured
// rather than stylistic. Gate-hop DISTANCE is symmetric, so that message names a pair
// and the caller may send either direction. A gate FEE is a property of the system a
// hull DEPARTS from: corr(fee, distance) = 0.124, the origin system explains 99.7% of
// the variance, and the same edge measured 6,760 credits one way against 5,313 the
// other. Sending a fee as a pair, or mirroring it, would misprice every return leg.
//
// fee_credits is the mean of that gate's observed jumps. Within-gate spread is 2.38%
// of a gate's own mean, so a single recorded jump already pins it to ~2.4% — against
// the flat charge's 15.2% mean absolute error. A non-positive fee is IGNORED by the
// solver (that crossing falls back to the flat charge); it is never charged as free.
type GateFee struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	System        string                 `protobuf:"bytes,1,opt,name=system,proto3" json:"system,omitempty"`
	FeeCredits    int64                  `protobuf:"varint,2,opt,name=fee_credits,json=feeCredits,proto3" json:"fee_credits,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GateFee) Reset() {
	*x = GateFee{}
...
func (x *GateFee) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GateFee) ProtoMessage() {}

func (x *GateFee) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_proto_routing_routing_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use GateFee.ProtoReflect.Descriptor instead.
func (*GateFee) Descriptor() ([]byte, []int) {
	return file_pkg_proto_routing_routing_proto_rawDescGZIP(), []int{18}
}

func (x *GateFee) GetSystem() string {
	if x != nil {
		return x.System
	}
	return ""
...
func (x *GateFee) GetFeeCredits() int64 {
	if x != nil {
		return x.FeeCredits
	}
	return 0
...
type TourTrade struct {
	...
	// is_stock marks a BUY-side tranche that is a WITHDRAWAL of factory output from
	// warehouse stock at cost basis (C1, sp-64je), not a market purchase.
	// expected_unit_price is the recorded cost basis (the flat source price the stock
	// pool was priced at), NOT a market ask — the executor withdraws via
	// TryReserveCargo/TransferCargo/ConfirmTransfer instead of buying, and books the
	// acquisition at basis. is_buy is true for a stock withdrawal (it acquires cargo);
	// is_deposit is always false. Additive to the deposit shape (a stock draw is a
	// buy-with-no-market-spend whose "cost" is the sunk basis); existing buy semantics
	// untouched — an old executor ignoring field 6 treats it as a normal buy at
	// expected_unit_price, which is the basis, so it still never overpays.
...
func (x *TourTrade) GetIsStock() bool {
	if x != nil {
		return x.IsStock
	}
	return false
...
// MarketAbsorption is the outstanding cross-container depth already committed at one
// (waypoint, good, side) pool that the solver NETS out of that market's available
// depth (sp-78ai L3). The Go daemon assembles these from the absorption ledger at plan
// assembly time — decaying EXECUTED recovery shadows on the fitted half-life curve
// Go-side, so the solver stays clock-free and the recovery curve is consumed in exactly
// one place. Two quantities with DIFFERENT netting semantics (design §2, the
// price-honesty split):
//   - units_planned: in-flight PLANNED reservations from OTHER containers. The market
//     cache is still pre-absorption, so these both CONSUME tranche capacity AND ADVANCE
//     the price schedule — the plan's first tranche at that sink prices at the decayed
//     step those earlier tranches leave behind, because someone is taking them there.
//   - units_recovering: the decayed EXECUTED recovery residual. The cache ALREADY shows
//     the crushed quote, so these consume tranche CAPACITY ONLY — pricing still starts
//     at the live quote's step 0, or the crush would be double-counted.
//
// Empty (the pre-sp-78ai shape) → no netting; the solver plans against full depth,
// byte-identical to today. Additive-field precedent (sp-bc27/sp-dchv): an old solver
// ignores field 6 and behaves exactly as it does now (version-skew safe).
type MarketAbsorption struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	WaypointSymbol  string                 `protobuf:"bytes,1,opt,name=waypoint_symbol,json=waypointSymbol,proto3" json:"waypoint_symbol,omitempty"`
	GoodSymbol      string                 `protobuf:"bytes,2,opt,name=good_symbol,json=goodSymbol,proto3" json:"good_symbol,omitempty"`
	Side            string                 `protobuf:"bytes,3,opt,name=side,proto3" json:"side,omitempty"`                                                // "sell" | "buy" (absorption.SideSell / SideBuy)
	UnitsPlanned    int32                  `protobuf:"varint,4,opt,name=units_planned,json=unitsPlanned,proto3" json:"units_planned,omitempty"`           // in-flight PLANNED — advance price schedule + consume capacity
	UnitsRecovering float64                `protobuf:"fixed64,5,opt,name=units_recovering,json=unitsRecovering,proto3" json:"units_recovering,omitempty"` // decayed EXECUTED residual — consume capacity ONLY
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *MarketAbsorption) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *MarketAbsorption) GetGoodSymbol() string {
	if x != nil {
		return x.GoodSymbol
	}
	return ""
...
func (x *MarketAbsorption) GetSide() string {
	if x != nil {
		return x.Side
	}
	return ""
...
func (x *MarketAbsorption) GetUnitsPlanned() int32 {
	if x != nil {
		return x.UnitsPlanned
	}
	return 0
}

func (x *MarketAbsorption) GetUnitsRecovering() float64 {
	if x != nil {
		return x.UnitsRecovering
	}
	return 0
}

// StockSource is one planner-visible-stock offer (C1, sp-64je): factory output
// deposited into a warehouse at a recorded cost basis that the tour may WITHDRAW
// instead of buying our own output at the laddered market ask. The mirror image of
// DepositCandidate — a zero-ask SOURCE rather than a synthetic sink. The Go daemon
// assembles these from the running warehouse op: units_available is the on-hand
// stock net of outstanding cross-tour reservations, unit_ask is the weighted-average
// cost basis. The solver treats (storage_waypoint, good_symbol) as a flat BUY pool
// priced at unit_ask (no depth decay — a withdrawal has no market price impact — and
// no A-cap, bounded by units_available), and prices withdrawal legs against real
// market buys on margin so the tour draws from stock only when it is the cheaper
// acquisition (the export-ask-subsidy inversion becomes structurally impossible).
type StockSource struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	GoodSymbol      string                 `protobuf:"bytes,1,opt,name=good_symbol,json=goodSymbol,proto3" json:"good_symbol,omitempty"`
	UnitsAvailable  int32                  `protobuf:"varint,2,opt,name=units_available,json=unitsAvailable,proto3" json:"units_available,omitempty"`   // reservable on-hand stock (Go-side: available minus outstanding reservations)
	UnitAsk         int32                  `protobuf:"varint,3,opt,name=unit_ask,json=unitAsk,proto3" json:"unit_ask,omitempty"`                        // flat source price = recorded cost basis
	StorageWaypoint string                 `protobuf:"bytes,4,opt,name=storage_waypoint,json=storageWaypoint,proto3" json:"storage_waypoint,omitempty"` // the warehouse hull's waypoint (the source node)
	StorageSystem   string                 `protobuf:"bytes,5,opt,name=storage_system,json=storageSystem,proto3" json:"storage_system,omitempty"`       // system the warehouse is in (makes the source routable)
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *StockSource) GetUnitsAvailable() int32 {
	if x != nil {
...
func (x *StockSource) GetUnitAsk() int32 {
	if x != nil {
		return x.UnitAsk
	}
	return 0
...
func (x *StockSource) GetStorageWaypoint() string {
	if x != nil {
		return x.StorageWaypoint
	}
	return ""
...
func (x *StockSource) GetStorageSystem() string {
	if x != nil {
		return x.StorageSystem
	}
	return ""
...
type OptimizeTradeTourRequest struct {
	...
	// absorption carries per-(waypoint,good,side) outstanding cross-container depth the
	// solver nets out of available tranche capacity (sp-78ai L3). Empty (the pre-sp-78ai
	// shape) → no netting, plans against full depth unchanged.
	Absorption []*MarketAbsorption `protobuf:"bytes,6,rep,name=absorption,proto3" json:"absorption,omitempty"`
	// stock_sources offers warehouse stock as zero-ask-at-basis withdrawal sources
	// (C1, sp-64je). Empty (the pre-C1 shape) → no stock legs, the solver plans against
	// market buys unchanged. Additive-field precedent (sp-dchv/sp-78ai): an old solver
	// ignores field 7 and behaves exactly as it does now (version-skew safe).
	StockSources  []*StockSource `protobuf:"bytes,7,rep,name=stock_sources,json=stockSources,proto3" json:"stock_sources,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *OptimizeTradeTourRequest) GetAbsorption() []*MarketAbsorption {
	if x != nil {
		return x.Absorption
	}
	return nil
...
func (x *OptimizeTradeTourRequest) GetStockSources() []*StockSource {
	if x != nil {
		return x.StockSources
	}
	return nil
}
...
func (x *OptimizeTradeTourResponse) Reset() {
	*x = OptimizeTradeTourResponse{}
	mi := &file_pkg_proto_routing_routing_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}
...
func (*OptimizeTradeTourResponse) Descriptor() ([]byte, []int) {
	return file_pkg_proto_routing_routing_proto_rawDescGZIP(), []int{28}
}
...
const file_pkg_proto_routing_routing_proto_rawDesc = "" +
	...
	"\n" +
	"is_deposit\x18\x05 \x01(\bR\tisDeposit\x12\x19\n" +
	"\bis_stock\x18\x06 \x01(\bR\aisStock\"\xf3\x01\n" +
	"\fTradeTourLeg\x12'\n" +
	"\x0fwaypoint_symbol\x18\x01 \x01(\tR\x0ewaypointSymbol\x12#\n" +
	...
	"\x0estorage_system\x18\x05 \x01(\tR\rstorageSystem\"\xc0\x01\n" +
	"\x10MarketAbsorption\x12'\n" +
	"\x0fwaypoint_symbol\x18\x01 \x01(\tR\x0ewaypointSymbol\x12\x1f\n" +
	"\vgood_symbol\x18\x02 \x01(\tR\n" +
	"goodSymbol\x12\x12\n" +
	"\x04side\x18\x03 \x01(\tR\x04side\x12#\n" +
	"\runits_planned\x18\x04 \x01(\x05R\funitsPlanned\x12)\n" +
	"\x10units_recovering\x18\x05 \x01(\x01R\x0funitsRecovering\"\xc4\x01\n" +
	"\vStockSource\x12\x1f\n" +
	"\vgood_symbol\x18\x01 \x01(\tR\n" +
	"goodSymbol\x12'\n" +
	"\x0funits_available\x18\x02 \x01(\x05R\x0eunitsAvailable\x12\x19\n" +
	"\bunit_ask\x18\x03 \x01(\x05R\aunitAsk\x12)\n" +
	"\x10storage_waypoint\x18\x04 \x01(\tR\x0fstorageWaypoint\x12%\n" +
	"\x0estorage_system\x18\x05 \x01(\tR\rstorageSystem\"\xab\x03\n" +
	...
	"\x12deposit_candidates\x18\x05 \x03(\v2\x19.routing.DepositCandidateR\x11depositCandidates\x129\n" +
	"\n" +
	"absorption\x18\x06 \x03(\v2\x19.routing.MarketAbsorptionR\n" +
	"absorption\x129\n" +
	"\rstock_sources\x18\a \x03(\v2\x14.routing.StockSourceR\fstockSources\"\xa6\x03\n" +
...
var file_pkg_proto_routing_routing_proto_msgTypes = make([]protoimpl.MessageInfo, 31)
var file_pkg_proto_routing_routing_proto_goTypes = []any{
	...
	(*GateFee)(nil),                    // 19: routing.GateFee
	(*InterSystemHopDistance)(nil),     // 20: routing.InterSystemHopDistance
	(*TourTrade)(nil),                  // 21: routing.TourTrade
	(*TradeTourLeg)(nil),               // 22: routing.TradeTourLeg
	(*RejectedTour)(nil),               // 23: routing.RejectedTour
	(*TourWaypoint)(nil),               // 24: routing.TourWaypoint
	(*DepositCandidate)(nil),           // 25: routing.DepositCandidate
	(*MarketAbsorption)(nil),           // 26: routing.MarketAbsorption
	(*StockSource)(nil),                // 27: routing.StockSource
	(*OptimizeTradeTourRequest)(nil),   // 28: routing.OptimizeTradeTourRequest
	(*OptimizeTradeTourResponse)(nil),  // 29: routing.OptimizeTradeTourResponse
	nil,                                // 30: routing.PartitionFleetRequest.ShipConfigsEntry
	nil,                                // 31: routing.PartitionFleetResponse.AssignmentsEntry
}
var file_pkg_proto_routing_routing_proto_depIdxs = []int32{
	...
	30, // 8: routing.PartitionFleetRequest.ship_configs:type_name -> routing.PartitionFleetRequest.ShipConfigsEntry
	2,  // 9: routing.PartitionFleetRequest.all_waypoints:type_name -> routing.Waypoint
	31, // 10: routing.PartitionFleetResponse.assignments:type_name -> routing.PartitionFleetResponse.AssignmentsEntry
	...
	20, // 13: routing.TourConstraints.inter_system_hops:type_name -> routing.InterSystemHopDistance
	19, // 14: routing.TourConstraints.gate_fees:type_name -> routing.GateFee
	21, // 15: routing.TradeTourLeg.trades:type_name -> routing.TourTrade
	15, // 16: routing.OptimizeTradeTourRequest.snapshot:type_name -> routing.MarketGoodSnapshot
	16, // 17: routing.OptimizeTradeTourRequest.ship:type_name -> routing.TourShip
	18, // 18: routing.OptimizeTradeTourRequest.constraints:type_name -> routing.TourConstraints
	24, // 19: routing.OptimizeTradeTourRequest.waypoints:type_name -> routing.TourWaypoint
	25, // 20: routing.OptimizeTradeTourRequest.deposit_candidates:type_name -> routing.DepositCandidate
	26, // 21: routing.OptimizeTradeTourRequest.absorption:type_name -> routing.MarketAbsorption
	27, // 22: routing.OptimizeTradeTourRequest.stock_sources:type_name -> routing.StockSource
	22, // 23: routing.OptimizeTradeTourResponse.legs:type_name -> routing.TradeTourLeg
	23, // 24: routing.OptimizeTradeTourResponse.top_rejected:type_name -> routing.RejectedTour
	12, // 25: routing.PartitionFleetRequest.ShipConfigsEntry.value:type_name -> routing.ShipConfig
	14, // 26: routing.PartitionFleetResponse.AssignmentsEntry.value:type_name -> routing.ShipTour
	1,  // 27: routing.RoutingService.PlanRoute:input_type -> routing.PlanRouteRequest
	5,  // 28: routing.RoutingService.OptimizeTour:input_type -> routing.OptimizeTourRequest
	7,  // 29: routing.RoutingService.OptimizeFueledTour:input_type -> routing.OptimizeFueledTourRequest
	11, // 30: routing.RoutingService.PartitionFleet:input_type -> routing.PartitionFleetRequest
	28, // 31: routing.RoutingService.OptimizeTradeTour:input_type -> routing.OptimizeTradeTourRequest
	3,  // 32: routing.RoutingService.PlanRoute:output_type -> routing.PlanRouteResponse
	6,  // 33: routing.RoutingService.OptimizeTour:output_type -> routing.OptimizeTourResponse
	8,  // 34: routing.RoutingService.OptimizeFueledTour:output_type -> routing.OptimizeFueledTourResponse
	13, // 35: routing.RoutingService.PartitionFleet:output_type -> routing.PartitionFleetResponse
	29, // 36: routing.RoutingService.OptimizeTradeTour:output_type -> routing.OptimizeTradeTourResponse
