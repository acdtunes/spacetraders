 ...
 *  no transit → unchanged; now < arrival → IN_TRANSIT; now >= arrival → IN_ORBIT (single atomic
 *  status flip). In BOTH the in-flight and arrived cases nav.waypointSymbol is the DESTINATION — the
 *  real-API contract (see below) — with the origin available on route via the transit. Reads REAL
 *  wall time by default (arrivals live on the real clock so the twin's status agrees with the daemon's
 *  real-time detection). route stays populated in flight. */
export function resolveNav(ship: Ship, transit: TransitState | undefined, now: Date = new Date()): Ship {
  ...
  return {
    ...
    nav: {
      ...
      // Real-API fidelity: an IN_TRANSIT ship's `waypointSymbol` is its DESTINATION (the waypoint it is
      // heading to), NOT the origin — the origin lives on route.origin. The daemon depends on this: it
      // maps nav.waypointSymbol straight onto Ship.currentLocation (adapters/api/ship_dto.go), and its
      // domain invariant is "while IN_TRANSIT, CurrentLocation() is the destination"
      // (application/ship/arrival_wait.go). Serving the ORIGIN here made a REBOOTED daemon re-adopt an
      // in-flight hull as if it were parked back at the shipyard origin, so the income coordinator saw
      // the hull's real destination hub as unserved and sent a second hauler there → two haulers on one
      // hub (income-restart-mid-transit's distinct-hubs failure). Only `status` flips at arrival.
