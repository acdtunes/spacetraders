...
export function applyReport(world: World, report: ReportInput): MutationLogEntry | null {
  switch (report.call) {
    ...
    case 'bootstrap-complete':
      // The bootstrap coordinator reached terminal COMPLETE (gate built + standing economy handed off)
      // and is exiting (actComplete res.Done=true). Flip the `done` observable exactly-once — the paired
      // flag for this report, mirroring the launch-* seams above. A repeat report is a pure no-op.
