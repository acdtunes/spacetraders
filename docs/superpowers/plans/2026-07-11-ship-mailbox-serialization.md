# Per-Ship Mailbox Serialization Implementation Plan (sp-eum3)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Every write to a ship's operational state flows through that ship's mailbox — a lazily-spawned per-`(playerID, shipSymbol)` goroutine that executes turns FIFO — so the owning container, the async arrival/cooldown scheduler, and the stuck-ship sweeper can no longer clobber each other's rows; the vestigial `ships.version` column becomes conflict-detection telemetry that proves it.

**Architecture:** Ship state lives in the DB (`ships` table); `api.ShipRepository.Save` (`ship_repository.go:826`) is a full-row `UpdateAll` upsert with no locking — last write wins. The confirmed race class (sp-n7yp dockrace; regression tests in `run_trade_route_coordinator_dockrace_test.go`) is the owning container's verb (`Dock`/`Navigate`/...) racing the `ShipStateScheduler`'s async `handleArrival`/`handleCooldownClear`/`sweepStuckShips` writes; a stale full-row `Save` can even clobber a concurrent column-scoped claim/release. The mailbox lives INSIDE `api.ShipRepository`, so all five mutating verbs are serialized with zero call-site changes; a new narrow `navigation.ShipMutator` port (`Mutate(ctx, playerID, symbol, op, fn)`) gives external read-modify-write flows the same turn semantics, and each turn re-finds fresh row state before mutating (serialization without re-find would still write stale snapshots). Assignment writes (`ClaimShip`/`ReserveForCaptain`/etc.) stay outside — they are already column-scoped `Updates` under `SELECT ... FOR UPDATE` (`ship_repository.go:1002`).

**Tech Stack:** Go 1.24, stdlib channels/goroutines, GORM (postgres + sqlite), `supervise.CapturePanic` from sp-i01z, existing sqlite test harness (`database.NewTestConnection()`).

## Global Constraints

- **Prerequisite: the sp-i01z supervision plan is merged** (`internal/infrastructure/supervise` must exist — the mailbox actor loop uses `CapturePanic`; the scheduler file was already touched there).
...
- Commit messages reference the bead: e.g. `feat(daemon): per-ship mailbox core (sp-eum3)`.
...
- Behavior preservation rule: verbs keep their exact signatures and their "Save failure is a logged warning, not an error" semantics; `ClaimShip`/assignment methods are untouched.

## Race classes this plan closes

| Race | Evidence | Closed by |
|---|---|---|
| Verb (Dock) vs async arrival timer writing the same row | sp-n7yp; `run_trade_route_coordinator_dockrace_test.go:20-27`; `ship_state_scheduler.go:95` TOCTOU | both sides become serialized turns (Tasks 3-5) |
| Stale full-row verb `Save` clobbering a concurrent column-scoped claim/release | `Save` writes ALL columns from the caller's snapshot (`shipToModel` includes assignment fields) | turns re-find fresh state; the transition is applied to the fresh row (Tasks 3-4) |
| Sweeper vs owning container (same TOCTOU as arrival) | `sweepStuckShips` (`ship_state_scheduler.go:275`) does Find→Arrive→Save on ships a container may be flying | sweeper writes become `Mutate` turns (Task 5) |
| Any not-yet-migrated writer still doing Find→mutate→Save | ~20 application files call `shipRepo.Save` (rollout bead sp-wa7c) | version-CAS telemetry makes every remaining clobber loud + counted (Task 6) |
...
- Create: `internal/adapters/api/ship_repository_mutate_test.go` — sqlite-backed Mutate + concurrency proof
- Create: `internal/adapters/api/ship_repository_serialized_verbs_test.go`
- Create: `internal/adapters/api/ship_repository_version_test.go`
- Modify: `internal/domain/navigation/ports.go` — `ShipMutator` port + `ErrSkipSave` sentinel
- Modify: `internal/domain/navigation/ship.go` — `AdoptState`, `PersistedVersion`/`SetPersistedVersion`
- Modify: `internal/adapters/api/ship_repository.go` — `mailboxes` field, `Mutate`, the 5 verbs, version-CAS `Save`, `modelToDomain` version threading
- Modify: `internal/adapters/grpc/ship_state_scheduler.go` — ctor gains `navigation.ShipMutator`; `handleArrival`/`handleCooldownClear`/`sweepStuckShips` become turns
...
### Task 1: Mailbox core (`shipMailboxes`)
...
- Produces (package-internal; Tasks 2-4 call it): `newShipMailboxes() *shipMailboxes`; `func (m *shipMailboxes) run(ctx context.Context, playerID shared.PlayerID, symbol, op string, fn func(ctx context.Context) error) error`. Guarantees: turns for one `(playerID, symbol)` execute strictly FIFO on one goroutine; turns for different ships run independently; a panicking turn returns `*supervise.PanicError` to its caller and the actor survives; re-entrant `run` for the same ship inside a turn fails fast with `errMailboxReentrant`; a caller whose ctx dies while queued gets `ctx.Err()` and its turn is skipped.
...
// (ship_repository.go:826, no version guard) otherwise leaves racy — the
...
### Task 2: `ShipMutator` port + `Mutate` implementation
...
**Interfaces:**
- Consumes: Task 1 `shipMailboxes.run`; existing `FindBySymbol` (`ship_repository.go:88`), `Save` (`:826`)
- Produces:
...
// newMutateTestRepo mirrors newDedicationTestRepo (ship_repository_claim_dedication_test.go:19)
// but with a waypoint-provider stub because Mutate's FindBySymbol path runs
// modelToDomain.
func newMutateTestRepo(t *testing.T) (*ShipRepository, *gorm.DB, shared.PlayerID) {
	t.Helper()
...
func TestMutate_ConcurrentTurnsLoseNoUpdates(t *testing.T) {
	repo, db, pid := newMutateTestRepo(t)
...
// semantics; compile-time check below.
...
(The `var _` check goes at file scope, not inside the func.)
...
### Task 3: Serialize `Dock` and `Orbit` (arrival-reconcile pair) + `Ship.AdoptState`
...
**Interfaces:**
- Consumes: Task 1 mailbox, `FindBySymbol`, `Save`, `domainPorts.APIClient.DockShip/OrbitShip`, `player.PlayerRepository.FindByID`
- Produces: `func (s *Ship) AdoptState(other *Ship)` (Task 4 reuses it). Verb signatures unchanged: `Dock(ctx, ship, playerID) error`, `Orbit(ctx, ship, playerID) error`.
...
func TestDock_ReconcilesFreshInTransitRow(t *testing.T) {
	...
	require.NoError(t, repo.db.Model(&persistence.ShipModel{}).
		...
		Updates(map[string]interface{}{"nav_status": "IN_TRANSIT", "arrival_time": past}).Error)
...
(`repo.db` is the unexported field — these tests are in package `api`, so direct access is fine, matching the dedication tests' style. Reuse `stubWaypoints`/`seedShip` from Task 2's file — same package.)
...
### Task 4: Serialize `Navigate`, `Refuel`, `SetFlightMode`
...
**Interfaces:**
- Consumes: Tasks 1-3 (`mailboxes.run`, `AdoptState`); `apiClient.NavigateShip/RefuelShip/SetFlightMode`; `r.arrivalScheduler`
...
func (a *stubAPI) SetFlightMode(_ context.Context, _ string, _ string, _ string) error { return nil }
...
(SetFlightMode's API call inside the turn is a deliberate exception to the Task 3 note: the equality short-circuit needs fresh state BEFORE deciding to call, and mode changes are rare.)
...
### Task 5: Route the `ShipStateScheduler` through `Mutate`
...
**Interfaces:**
- Consumes: `navigation.ShipMutator`, `navigation.ErrSkipSave` (Task 2)
...
Run `rtk grep -rn 'NewShipStateScheduler(' --include='*.go' internal/ cmd/` and update every test constructor to pass a fourth argument (a `fakeMutator` or `nil` where the sweeper/timers aren't exercised — note the Task 5 handlers deref it, so tests exercising them must pass a fake).
...
### Task 6: `ships.version` conflict telemetry (detection-only CAS)

**Files:**
- Modify: `internal/domain/navigation/ship.go` (`persistedVersion` field + accessors)
- Modify: `internal/adapters/api/ship_repository.go` (`modelToDomain` line ~1301 threads the version; `shipToModel` line 669 writes `persisted+1`; `Save` line 826 gains the guarded upsert + fallback)
- Modify: `internal/adapters/metrics/prometheus_collector.go` + `container_metrics.go` (counter + shim, same pattern as sp-i01z's daemon-component metric)
- Test: `internal/adapters/api/ship_repository_version_test.go`

**Interfaces:**
- Consumes: existing `Version` column (`models.go:170`, today hardcoded `1` at `ship_repository.go:675` and `:1458` — vestigial)
- Produces: `ship.PersistedVersion() int` / `ship.SetPersistedVersion(v int)`; metric `spacetraders_ship_version_conflicts_total`; package counter `shipVersionConflicts atomic.Int64` (test/introspection hook). **Behavior is preserved**: a detected conflict logs ERROR + counts + falls back to today's last-write-wins upsert. This is the tripwire that (a) proves the mailbox works (conflicts ≈ 0 on migrated paths) and (b) makes every not-yet-migrated writer (bead sp-wa7c) loud.
...
// Serialized turns never conflict: Mutate re-finds fresh (latest version)
// inside the turn, so the CAS always matches. This is the proof wire.
...
Run: `rtk go test ./internal/adapters/api/ -run 'TestSave_Detects|TestMutate_TurnsNever|TestSave_Unknown' -v`
Expected: FAIL (PersistedVersion undefined)

- [ ] **Step 3: Implement**

`ship.go` — field + accessors (near the other infrastructure-ish fields):
...
```go
	...
	// for the Save CAS tripwire (sp-eum3): it is NOT domain state and has no
	// behavior here.
...
// (0 = unknown/API-born). See sp-eum3 conflict telemetry.
...
3. Package counter (top of `ship_mailbox.go` or `ship_repository.go`): `var shipVersionConflicts atomic.Int64` (import `sync/atomic`).
...
// since this entity was loaded. That is DETECTION-ONLY telemetry (sp-eum3):
// we count + log, then apply the legacy last-write-wins upsert so behavior
// is unchanged while bead sp-wa7c migrates the remaining writers to Mutate.
func (r *ShipRepository) Save(ctx context.Context, ship *navigation.Ship) error {
	...
	if loaded := ship.PersistedVersion(); loaded > 0 {
		...
		log.Printf("ERROR: ship %s save conflict — row version moved past %d (writer outside the mailbox, see sp-wa7c); applying last-write-wins fallback",
...
Contingency (document in the commit if hit): if the sqlite driver rejects `clause.OnConflict.Where`, replace the guarded branch with an explicit two-step inside the same call: `UPDATE ships SET ... WHERE ship_symbol=? AND player_id=? AND version=?` via `.Model(&persistence.ShipModel{}).Where(...).Updates(model)` with `Select("*")`, falling back to the upsert when `RowsAffected == 0` AND distinguishing missing-row via a `First` probe. The tests in Step 1 define the contract either way.

5. Metrics (mirror sp-i01z Task 3 exactly): in `prometheus_collector.go` add `ShipWriteConflictRecorder interface { RecordShipVersionConflict() }` + the type-asserted `RecordShipVersionConflict()` shim; in `container_metrics.go` add field `shipVersionConflicts prometheus.Counter` built with `prometheus.NewCounter(prometheus.CounterOpts{Namespace: namespace, Subsystem: "ship", Name: "version_conflicts_total", Help: "Ship row writes that raced past their loaded version (writers outside the mailbox)"})`, register it, and implement the method. Import `metrics` in `ship_repository.go` (check for an import cycle: `internal/adapters/metrics` must not import `internal/adapters/api` — it doesn't today; if the compiler disagrees, call the shim through a package-level `var onVersionConflict = func(){}` hook set from main instead).

- [ ] **Step 4: Run tests**

Run: `rtk go test -race ./internal/adapters/api/ && rtk go test ./internal/adapters/metrics/`
Expected: PASS (including all pre-existing repo tests — the CAS branch only activates for entities with a known version, and every load path now sets one, so watch for surprises in claim/dedication tests and fix the SEED data, not the guard)

- [ ] **Step 5: Commit**

```bash
rtk git add internal/domain/navigation/ship.go internal/adapters/api/ internal/adapters/metrics/
rtk git commit -m "feat(daemon): ships.version becomes a live CAS tripwire — conflicts counted+logged, behavior preserved (sp-eum3)"
```
...
`bd update sp-eum3 --append-notes="implementation complete: mailbox core, Mutate port, 5 verbs serialized, scheduler turns, version tripwire"`, then the repo session-close protocol (commit, `bd dolt push`, `git push`). After deploy, watch `spacetraders_ship_version_conflicts_total` — a nonzero rate points at the exact writers bead sp-wa7c migrates next.

---

## Deliberately out of scope (beads filed)

- **sp-wa7c** — migrate the ~20 external `FindBySymbol → mutate → Save` call sites (list in the bead: gas siphon/transfer, cargo_transaction, jump_ship, outfitting, route_executor, worker lifecycle managers, coordinators' direct Saves, `refresh_ship`, `purchase_ship`, `scout_markets`, `container_ops_scout_posts`, `container_runner` release path, `daemon_server`) to `ShipMutator.Mutate`. The version tripwire ranks them by actual conflict frequency.
- `SaveAll` (`ship_repository.go:850`) keeps legacy semantics (no known callers found; used by bulk sync at most) — flag in sp-wa7c.
- Sync paths (`SyncShipFromAPI`/`SyncAllFromAPI`, the `UpdateAll` writers at `:1643`/`:1706`) stay outside the mailbox: startup/new-hull API imports, not steady-state contention. The tripwire will say if that assumption is wrong.
...
1. **Mailbox inside the repository, not a separate service**: all five verbs get serialized with zero call-site churn, and the repo is the only component that owns both the API client and the row — the turn = the whole read-modify-write.
2. **Narrow `ShipMutator` port instead of widening `navigation.ShipRepository`**: the big port has many fakes across 169 test files; adding a method breaks all of them for zero benefit.
3. **Re-find fresh + `AdoptState` copy-back**: serialization alone still writes stale snapshots (phantom arrival, stale assignment columns). Fresh-apply is the semantic fix; copy-back preserves every caller's "read the entity after the verb" contract.
4. **API call outside the turn (except SetFlightMode)**: rate-limited API calls can stall 30s under burst backoff; holding the mailbox through them would starve arrival turns. Dock/orbit are idempotent; navigate's API-then-turn ordering matches today's API-then-save ordering.
5. **Version CAS is detection-only**: flipping conflicts to hard failures would change ~20 call sites' error behavior in one shot. Loud-but-preserving converts the migration (sp-wa7c) from a leap into a measured, metric-guided walk.
6. **No actor reaping**: bounded by fleet size; a teardown/enqueue race is real complexity for ~zero memory.
