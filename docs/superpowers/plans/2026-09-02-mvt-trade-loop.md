...
8. `ActualJumps += (ActualNext != X)`, `LoopJumps += (LoopNext != X)`. `ActualMargin += yield-by-h in [t, t+Horizon)`; `LoopMargin += unitsSoldBy h in [t, t+Horizon) × (margin/units of yield(LoopNext, t, t+Horizon))` (0 when that system had no sells). `Stranded = LoopNext != X && yield(LoopNext, t, t+Horizon).units == 0`.
