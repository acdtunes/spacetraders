...
- **RULINGS #2 — derived, never stored.** The wave predicate is recomputed from durable facts on every tick in every consumer. Nothing about the wave is persisted, cached, or carried between ticks: no cursor, no high-water mark, no streak on the predicate.
